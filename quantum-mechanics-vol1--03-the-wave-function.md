...
**Project:** The 1D Quantum Sandbox · **Module this chapter adds:** the wave-function container (parallel `{re, im}` Float64Arrays), h-weighted normalization, and the `|ψ|²` / Re ψ / Im ψ D3 view — the `01-probability-explorer.html` that later chapters reuse.

**Tool:** Claude Code (default; Codex CLI or Cowork work identically — in Cowork, open the explorer in the built-in preview; the numerical check still runs under `node`).
**Skill level:** Intermediate — complex arrays stored as two real arrays, and the normalization integral must carry its `h`.

**Setup — confirm before you start:**

- [ ] `src/grid.js` (Chapter 2) and `src/constants.js` (Chapter 0) exist and pass their checks.
- [ ] Node ≥ 18 (plain ES modules); a browser to view the probability explorer.
- [ ] The standing rule is in `CLAUDE.md`.

**The Task — paste into your CLI:**

```
Read src/grid.js and src/constants.js (do NOT modify them). Create ONLY the new files below; leave all other files alone.

1. src/wavefn.js, exporting:
   - makePsi(grid) -> { re: Float64Array, im: Float64Array } of length grid.x.length (complex psi as parallel real arrays, per the project convention).
   - gaussianPacket(grid, { x0, a, k0 }) -> psi with Re = A e^{-(x-x0)^2/2a^2} cos(k0 x), Im = A e^{-(x-x0)^2/2a^2} sin(k0 x).
   - probabilityDensity(psi) -> Float64Array of re^2 + im^2.
   - normalize(psi, h): divide by sqrt(sum_j (re_j^2+im_j^2)*h) so that sum_j |psi_j|^2 * h = 1. The *h is mandatory (Chapter 3, Born rule).
   - current(psi, grid): the probability current J = (hbar/m) Im(psi* dpsi/dx) by central difference.

2. Add to index.html a view "01-probability-explorer" plotting |psi|^2 (blue filled), Re psi (orange), Im psi (gray dashed) for a gaussianPacket.

3. test/check-wavefn.mjs that, for a gaussianPacket with a and k0 you choose:
   - asserts sum_j |psi_j|^2 * h = 1 to < 1e-6 after normalize.
   - computes sigma_x = sqrt(<x^2> - <x>^2) from |psi|^2 and asserts sigma_x = a/sqrt(2) to < 1%.
   - computes sigma_p from the momentum-space width and asserts sigma_x * sigma_p = hbar/2 to < 1% (the Gaussian saturates the Kennard bound).
   - asserts a purely real psi has current J = 0 everywhere.
   - prints PASS/FAIL with the numbers for each.

Then RUN `node test/check-wavefn.mjs` and show the output. Stop.
```

**Expected output:** `src/wavefn.js`, an `01-probability-explorer` view in `index.html`, and a printed `PASS norm = 1.0000000`, `PASS σx = a/√2`, `PASS σxσp = ℏ/2`, `PASS real ψ ⇒ J = 0`.

**The golden check (what makes this simulation trustworthy):** ∑ⱼ|ψⱼ|²·h = 1 to < 1e-6 after normalization, and for the Gaussian ψ = (πa²)^(−1/4) e^(−x²/2a²) e^(ik₀x) the spreads obey σx = a/√2 and σx·σp = ℏ/2 to < 1% — the minimum-uncertainty relation derived in this chapter.

**What to inspect:** that `|ψ(0)|²` may exceed 1 (it is a density per nm, not a probability — a units fact, not a bug); that Im ψ leads Re ψ by a quarter cycle; that the normalization readout is exactly 1, not 250.

**If it goes wrong:** the signature failure is dropping the `h` in the normalization sum, which makes the norm read 1/h (about 250 for h = 0.004 nm) instead of 1. If the indicator reads a large constant, multiply the summed density by `h`; do not "rescale ψ until it looks normalized."

**CLAUDE.md note:** Already covered by the project rule (the ∑|ψ|²·h = 1 check is exactly the standing normalization discipline).
