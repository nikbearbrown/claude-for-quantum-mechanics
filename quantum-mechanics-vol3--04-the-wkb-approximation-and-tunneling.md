...
**Project:** Model a Real Quantum System · **Module this chapter adds:** a WKB tunneling calculator (Gamow factor T ≈ e^{−2γ}) validated against the exact rectangular-barrier transmission.

**Tool:** Claude Code (default; Codex CLI or Cowork work identically — pure numpy, no environment-specific calls).
**Skill level:** Intermediate — one integral and one closed-form comparison; the subtlety is the O(1) prefactor.
...
```
Add methods/wkb.py to the qm-real-system repo. Import epsilon.py, validate.py, report.py.
Do not modify earlier chapters' modules.

Physics: an electron (m_e = 9.11e-31 kg) with E = 1 eV hits a rectangular barrier of height
V0 = 5 eV and width L = 5 angstrom. kappa = sqrt(2 m_e (V0-E))/hbar.

1. FIRST estimate the small parameter. WKB tunneling is controlled by the thickness in decay
   lengths, gamma = kappa*L; the method is valid when gamma >> 1 and breaks as E -> V0. Print
   epsilon = 1/(kappa*L) via epsilon.estimate_epsilon("WKB", 1.0/(kappa*L),
   "epsilon = 1/(kappa L) not small, or E -> V0") and warn if it exceeds 0.3.

2. THEN implement:
   - T_wkb(E, V0, L)  = exp(-2*kappa*L)   for a general barrier via gamma = integral of
     sqrt(2 m (V-E))/hbar dx (here rectangular, so kappa*L).
   - T_exact(E, V0, L) = [1 + V0^2 * sinh^2(kappa L) / (4 E (V0-E))]^(-1).
   Compute both at the stated numbers (expect T_wkb ~ 3.7e-5, T_exact ~ 9.5e-5).

3. THEN RUN the verification. WKB gets the EXPONENT right and misses only the analytic prefactor
   16 E (V0-E)/V0^2. So check the ratio, not the raw T:
   - ratio = T_exact / T_wkb ; predicted_prefactor = 16*E*(V0-E)/V0^2 (= 2.56 here).
   - validate.check("WKB tunneling prefactor", ratio, predicted_prefactor, 0.05, "").
   Then sweep E from 0.5 to 4.9 eV and confirm log10(T_wkb) and log10(T_exact) stay PARALLEL
   (constant offset) until E -> V0, where WKB kinks. Print a five_move_report; the breakdown
   line notes the turning-point (Airy) corrections and the E->V0 failure.

STOP and show me methods/wkb.py and the run output. Delete nothing.
```

**Expected output:** `methods/wkb.py`; `epsilon[WKB] = 0.196` (1/γ with γ = κL ≈ 5.1); T_wkb ≈ 3.7×10⁻⁵, T_exact ≈ 9.5×10⁻⁵; and `WKB tunneling prefactor: predicted≈2.56 measured≈2.56 err<5% PASS`, plus a parallel-log-slope confirmation.

**The golden check (what makes this simulation trustworthy):** WKB must reproduce the *exponent* of the exact rectangular-barrier transmission $T_\text{exact}=[1+V_0^2\sinh^2(\kappa L)/(4E(V_0-E))]^{-1}$ exactly — the ratio $T_\text{exact}/T_\text{WKB}$ must equal the analytic prefactor $16E(V_0-E)/V_0^2 = 2.56$ to within 5%, while γ = κL ≈ 5.1 is safely large. On a log plot the two transmissions run parallel (same slope 2κ), which is why WKB nails physics spanning 24 decades even though it misses the O(1) prefactor.

**What to inspect:** is γ = κL genuinely ≫ 1 (so ε = 1/γ ≈ 0.2 is smallish)? Do the two log-transmission curves stay parallel until E approaches V₀? Does the WKB curve develop a kink at the barrier top where the exact one passes through smoothly — the documented failure as E → V₀?

**If it goes wrong:** if the ratio is not ≈ 2.56, check the factor of 2 in the exponent (T = e^{−2γ}, not e^{−γ}) — this is the most common sign/factor slip and you should surface it, not tune the tolerance. If you apply the tunneling formula near E = V₀ (κ → 0) and get T > 1, that is the method leaving its regime, exactly as the chapter warns; report it as a breakdown, not a bug to patch.

**CLAUDE.md note:** Already covered by the project rule (state ε = 1/κL, check against the analytic barrier limit, name the E→V₀ breakdown).
