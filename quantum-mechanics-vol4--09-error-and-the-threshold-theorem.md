...
**Project:** Reconstruct a Research Result · **Module this chapter adds:** `qrecon/qec.py` — the surface-code threshold scaling law p_L ≈ A(p/p_th)^⌈(d+1)/2⌉ with its suppression factor Λ, plus a 3-qubit stabilizer syndrome table: the tool for reconstructing a QEC-milestone paper's logical error rate and Λ.

**Tool:** Claude Code (default; Codex CLI or Cowork work identically — the syndrome simulation uses the same gate/tensor helpers you already built).
**Skill level:** Intermediate–Advanced — the scaling law is a formula, but the syndrome table requires stabilizer measurements on the 3-qubit code, and you must reason about the below-vs-above-threshold crossover.

**Setup — confirm before you start:**

- [ ] `qrecon/core.py` (tensor, density) and `qrecon/gates.py` (Chapter 4: Z, CNOT) exist.
- [ ] Python ≥3.10 with numpy.
- [ ] The standing project rule is in `CLAUDE.md`.

**The Task — paste into your CLI:**
```
Add a QEC module to my qrecon toolkit. Python 3 + numpy. Read qrecon/core.py and
qrecon/gates.py first and reuse tensor/gates; do not rewrite them. Create qrecon/qec.py
and tests/test_qec.py only. Two concepts: (a) the surface-code threshold scaling law,
(b) the 3-qubit bit-flip code syndrome.

In qrecon/qec.py implement:
  - p_logical(p, d, p_th=0.01, A=0.1): A*(p/p_th)**ceil((d+1)/2).
  - suppression(p, p_th=0.01): Lambda ~ p_th/p.
  - syndrome(error): for the 3-qubit code state alpha|000>+beta|111> with a single
    X error on qubit i (i in {none,1,2,3}), return the eigenvalues (m1,m2) of the
    stabilizers M1=Z1Z2 and M2=Z2Z3, computed WITHOUT measuring alpha,beta.

In tests/test_qec.py verify, printing numbers, this chapter's analytic facts:
  1. Below threshold (p=0.002 < p_th=0.01): p_logical DECREASES as d goes 3->5->7
     (bigger code is better).
  2. Above threshold (p=0.02 > p_th): p_logical INCREASES as d goes 3->5->7
     (bigger code is worse).
  3. At p = p_th: p_logical(d) = A = 0.1 for all d (the curves cross at threshold).
  4. Syndrome table: {none:(+1,+1), X1:(-1,+1), X2:(-1,-1), X3:(+1,-1)} -- four errors,
     four distinct syndromes, and both terms of the superposition give the SAME
     eigenvalue (alpha,beta never revealed).
  5. Willow-style extraction: with p_th=0.01 and reported Lambda=2.14, the implied
     physical error rate is p = p_th/Lambda = 0.0047 (print it; do NOT claim it equals
     any paper's value -- that is for me to compare).

Do not invent Google Willow's reported p_L or Lambda -- only I supply those from the
paper; your job is to compute the scaling and print it next to a placeholder for the
reported number.
When done, RUN the tests and print one line:
  "GOLDEN CHECK: below_thr_decreasing=<T/F> above_thr_increasing=<T/F> pL_at_thr=<...> syndromes_unique=<T/F> -> PASS/FAIL"
PASS requires below_thr_decreasing True, above_thr_increasing True, pL_at_thr=0.1, syndromes_unique True.
Show me the line, then STOP.
```

**Expected output:** `qrecon/qec.py`, `tests/test_qec.py`, and a line like `GOLDEN CHECK: below_thr_decreasing=True above_thr_increasing=True pL_at_thr=0.100000 syndromes_unique=True -> PASS`.

**The golden check (what makes this simulation trustworthy):** the scaling law p_L ≈ A(p/p_th)^⌈(d+1)/2⌉ makes the logical error rate fall with distance when p < p_th and rise when p > p_th, with all distances crossing at p_L = A when p = p_th; the suppression factor Λ ≈ p_th/p; and the 3-qubit code maps each single-qubit X error to a unique (M₁, M₂) syndrome without ever revealing the encoded amplitudes α, β.

**What to inspect:** below threshold, is each larger-d curve strictly lower? Do the four syndromes come out as the four distinct sign pairs? Does each stabilizer give the *same* eigenvalue on both |000⟩- and |111⟩-derived terms (so measuring it does not collapse the logical qubit)?

**If it goes wrong:** if p_L increases with d below threshold, the exponent ⌈(d+1)/2⌉ was mis-implemented (e.g., integer-divided wrong) — check it equals 2, 3, 4 for d = 3, 5, 7. If two errors share a syndrome, the stabilizers M₁ = Z₁Z₂ and M₂ = Z₂Z₃ were built on the wrong qubit pairs.

**CLAUDE.md note:** Already covered by the project rule — and note its clause: do not assert your computed p_L reproduces Willow's; print both numbers and let the human adjudicate.
