...
**Project:** Reconstruct a Research Result · **Module this chapter adds:** `qrecon/entanglement.py` — the coefficient-matrix determinant test, a Schmidt decomposition via SVD, and the entanglement entropy: the tool that identifies and verifies the resource state a paper claims to use.

**Tool:** Claude Code (default; Codex CLI or Cowork work identically — numpy's `linalg.svd` and `linalg.eigvalsh` are the only nonstandard calls, available everywhere).
**Skill level:** Intermediate — you must connect the SVD of the coefficient matrix to the reduced-state eigenvalues, and handle the `0·log0 = 0` convention.

**Setup — confirm before you start:**

- [ ] `qrecon/core.py` (Introduction) and `qrecon/density.py` (Chapter 1) exist.
- [ ] Python ≥3.10 with numpy.
- [ ] The standing project rule is in `CLAUDE.md`.

**The Task — paste into your CLI:**
```
Add an entanglement module to my qrecon toolkit. Python 3 + numpy. Read
qrecon/core.py and qrecon/density.py first and reuse partial_trace/purity; do not
rewrite them. Create qrecon/entanglement.py and tests/test_entanglement.py only.

In qrecon/entanglement.py implement, for a two-qubit pure state psi (length-4 vector):
  - coeff_matrix(psi): the 2x2 matrix C with C[i,j]=amplitude of |ij>.
  - det_test(psi): return det(C); |det(C)|>tol means entangled (rank-2), 0 means product.
  - schmidt(psi): return the sorted Schmidt coefficients lambda_k (eigenvalues of
    C^dagger C, equivalently of the reduced density matrix), via SVD of C.
  - entanglement_entropy(psi): S_E = -sum lambda_k log2 lambda_k, using the
    convention 0*log2(0)=0.

In tests/test_entanglement.py verify, printing numbers, this chapter's analytic facts:
  1. |Phi+>=(|00>+|11>)/sqrt2: det(C)=0.5 (entangled), Schmidt coeffs {0.5,0.5},
     S_E = 1.000 ebit exactly.
  2. Product state |+>|0>=(|00>+|10>)/sqrt2: det(C)=0 (separable), S_E = 0.
  3. The worked-example state |chi>=(sqrt3/2)|00>+(1/2)|11>: Schmidt coeffs {0.75,0.25},
     S_E = 0.811 ebits (within 1e-3).
  4. Consistency: schmidt(psi) equals the eigenvalues of
     partial_trace(density(psi), keep=0, dims=(2,2)) for 20 random two-qubit states.

Do not clamp tiny negative eigenvalues from round-off without saying so in a comment.
When done, RUN the tests and print one line:
  "GOLDEN CHECK: SE_Bell=<...> SE_product=<...> SE_chi=<...> schmidt_vs_rdm_maxerr=<...> -> PASS/FAIL"
PASS requires SE_Bell=1.000, SE_product=0.000, SE_chi within 1e-3 of 0.811, maxerr<1e-9.
Show me the line, then STOP.
```

**Expected output:** `qrecon/entanglement.py`, `tests/test_entanglement.py`, and a line like `GOLDEN CHECK: SE_Bell=1.000000 SE_product=0.000000 SE_chi=0.811278 schmidt_vs_rdm_maxerr=2.7e-16 -> PASS`.

**The golden check (what makes this simulation trustworthy):** a Bell state has Schmidt rank 2 with equal coefficients {½,½} and S_E = 1 ebit exactly; a product state has det(C) = 0 and S_E = 0; the partially entangled √3/2|00⟩+½|11⟩ has Schmidt coefficients {¾,¼} and S_E ≈ 0.811 ebits; and the Schmidt coefficients always equal the reduced density matrix's eigenvalues.

**What to inspect:** does det(C) vanish exactly for factorable states and stay nonzero for entangled ones? Do the Schmidt coefficients sum to 1 (they are a probability distribution)? Is S_E bounded by log₂2 = 1 ebit for two qubits?

**If it goes wrong:** S_E > 1 ebit for two qubits is impossible — you probably used natural log instead of log₂, or forgot to normalize the state. A NaN in S_E means a `log2(0)` slipped through; enforce the `0·log0 = 0` convention explicitly.
