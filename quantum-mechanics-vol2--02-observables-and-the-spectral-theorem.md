...
**Project:** Build the Atom · **Module this chapter adds:** the observable/measurement engine — diagonalize a Hermitian operator into real eigenvalues, orthonormal eigenstates, and projectors, then apply the Born rule. This is the routine that will later diagonalize the one-electron effective Hamiltonian for orbital energies.

**Tool:** Claude Code (default; Codex CLI or Cowork work identically — `numpy.linalg.eigh` behaves the same across all three).
**Skill level:** Intermediate — spectral decomposition and the amplitude-vs-probability distinction are where subtle bugs hide.

**Setup — confirm before you start:**
- [ ] `atom/states.py` (Chapter 1) and `atom/linalg.py` (Chapter 0) exist and their tests pass.
- [ ] Python ≥3.10 with numpy.
- [ ] Already covered by the project rule (confirm `CLAUDE.md` carries it).

**The Task — paste into your CLI:**
```
Add the observable/measurement engine. Read atom/linalg.py and atom/states.py to reuse is_hermitian and
inner/expval; do not modify them. Create ONLY atom/observables.py and tests/test_observables.py. Do not
delete anything.

In atom/observables.py implement, with docstrings, the spectral-theorem machinery from this chapter:
  - diagonalize(A)             -> (eigvals, eigvecs, projectors) for Hermitian A, using numpy.linalg.eigh;
                                  raise if A is not Hermitian to 1e-12. projectors[k] = |v_k><v_k|.
  - born_probabilities(A, psi) -> dict {eigenvalue: probability} using P(a_n) = ||P_n psi||^2.
  - spectral_expectation(A, psi) -> sum_n a_n * P(a_n)   (the probability-weighted average).

In tests/test_observables.py write a runnable check (asserts + prints) using the chapter's worked 2x2
observable B = [[2, 1+1j],[1-1j, 0]]:
  (a) eigenvalues are REAL: print PASS if max|Im(eigvals)| < 1e-12, and print the eigenvalues next to the
      chapter's analytic values 1+sqrt(3) and 1-sqrt(3), PASS if they match to 1e-10;
  (b) projector properties: print PASS if P_k @ P_k == P_k and sum_k P_k == I, both to 1e-12;
  (c) BORN + SPECTRAL AGREEMENT (the chapter's central identity): for psi = (|0>+|1>)/sqrt(2), print PASS
      if the Born probabilities sum to 1 to 1e-12 AND spectral_expectation(B, psi) equals the direct
      expval(B, psi) to 1e-12; print both numbers.

Then RUN: python tests/test_observables.py  and show me the PASS/FAIL lines. Stop after that.
```

**Expected output:** `atom/observables.py`, `tests/test_observables.py`, and printed lines such as `eigs(B)=[-0.732, 2.732] vs analytic [1-√3, 1+√3] -> PASS`, `P²=P and ΣP=I -> PASS`, `Σ aₙP(aₙ)=2.000 = ⟨B⟩=2.000 -> PASS`.

**The golden check (what makes this simulation trustworthy):** the Hermitian observable B = [[2, 1+i],[1−i, 0]] has eigenvalues exactly 1 ± √3 (both real, imaginary part < 1e-12), its projectors satisfy P²=P and ΣP=I, and the spectral sum Σ aₙP(aₙ) equals the direct ⟨ψ|B|ψ⟩ to machine precision — the chapter's worked example reproduced numerically.

**What to inspect:** that probabilities are `|amplitude|²`, not the raw complex amplitude (a probability must be real and non-negative — print and confirm); that ΣP(aₙ)=1 exactly; that eigenvalues come back real even though B has complex off-diagonal entries.

**If it goes wrong:** the classic bug is computing `P(a_n) = <a_n|psi>` (a complex amplitude) instead of `|<a_n|psi>|²` — you'll get complex "probabilities" or a sum that isn't 1. Fix by squaring the modulus; do not paper over a complex probability by taking its real part.
