...
**Project:** The QM Math Toolkit · **Module this chapter adds:** `units.py` — a units/dimension estimator that assembles characteristic scales (Bohr radius, Compton wavelength, de Broglie wavelength) from fundamental constants by dimensional analysis, the reflexes Vol. I·1 assumes from its first page.

**Tool:** Claude Code (default; Codex CLI or Cowork work identically — you may use `scipy.constants` for CODATA values; do not retype constants by hand).
**Skill level:** Beginner–Intermediate — dimensional analysis fixes the form; the check is matching CODATA to 3 significant figures.

**Setup — confirm before you start:**

- [ ] `qmtoolkit/scales.py` from Chapter 13 (shared unit-handling discipline; eV↔joule conversions).
- [ ] Python ≥3.10 with numpy, scipy, matplotlib (`scipy.constants` supplies CODATA values).
- [ ] Add the standing rule (bottom of this block) to `CLAUDE.md` if it isn't there yet.

**The Task — paste into your CLI:**

```
Add a units/dimension estimator to qmtoolkit. Vol. I Ch. 1 argues from scales: the atomic
size, the classical-vs-quantum crossover, the de Broglie wavelength — all fixed by
dimensional analysis before any Schrödinger equation is solved.

Create qmtoolkit/units.py using scipy.constants (hbar, m_e, e, epsilon_0, c, k_B):
  - bohr_radius(): a0 = 4π ε0 ħ^2 / (m_e e^2).
  - compton_wavelength(): λ_C = ħ / (m_e c).
  - de_broglie(E_eV, m=m_e): λ = h / sqrt(2 m E), E in eV converted to joules.
  - fine_structure(): α = e^2 / (4π ε0 ħ c).
Leave other files untouched.

Create tests/test_units.py checking closed-form / CODATA facts from this chapter:
  1. bohr_radius() ≈ 5.29e-11 m to 3 significant figures (0.529 Å).
  2. compton_wavelength() ≈ 3.86e-13 m to 3 sig figs.
  3. fine_structure() ≈ 1/137 (7.297e-3) to 3 sig figs, and is DIMENSIONLESS.
  4. Scale relation from the chapter: a0 = λ_C / α to 1e-6 (a0 α = λ_C), and
     de_broglie(1.0 eV electron) ≈ 1.23 nm to 3 sig figs.
  DERIVE targets from the constants; only the CODATA comparison values are looked up.

Then RUN `pytest -q`. Print each computed scale beside its accepted value, the a0 = λ_C/α
identity residual, and α, with PASS/FAIL. Do not touch unrelated files. Stop after the summary.
```

**Expected output:** `qmtoolkit/units.py` and `tests/test_units.py` added; `pytest` passes; printed `a0 = 5.29e-11 m PASS`, `λ_C = 3.86e-13 m PASS`, `α = 7.297e-3 ≈ 1/137 PASS`, `a0·α − λ_C = 3e-19 PASS`, `λ_dB(1 eV) = 1.23 nm PASS`.

**The golden check (what makes this routine trustworthy):** dimensional analysis reproduces the measured scales — `a_0 = 4πε₀ℏ²/(m_e e²) ≈ 5.29×10⁻¹¹ m`, `λ_C = ℏ/(m_e c) ≈ 3.86×10⁻¹³ m`, `α ≈ 1/137` (dimensionless) — and the exact scale relation `a_0 = λ_C/α` holds to `1e-6`, with the 1 eV electron de Broglie wavelength ≈1.23 nm.

**What to inspect:** each length should match CODATA to 3 significant figures (a mismatch by ~10³ or 10⁻³ is an SI-prefix slip; a mismatch by `2π` is an `h`-vs-`ℏ` confusion); `α` must be dimensionless (compute its units symbolically or confirm the value is ~0.0073); the `a_0 = λ_C/α` identity ties the three scales together and should be near the rounding floor.

**If it goes wrong:** if the Bohr radius is off by `2π`, an `h` slipped in where `ℏ` belongs (this chapter's named silent-`2π` error); if off by `10⁹`, a nm/m conversion. Pull constants from `scipy.constants`, not hand-typed digits, so CODATA precision is preserved and surfaced.
