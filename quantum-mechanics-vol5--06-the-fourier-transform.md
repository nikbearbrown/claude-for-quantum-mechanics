...
**Project:** The QM Math Toolkit · **Module this chapter adds:** `fft_tools.py` — a `numpy.fft` wrapper with an explicit, documented normalization convention, verified by "FFT of a Gaussian is a Gaussian" and Parseval/Plancherel, the position↔momentum machinery of I·8, III·5, and III·8.

**Tool:** Claude Code (default; Codex CLI or Cowork work identically — the discrete FFT is the same everywhere; only the normalization convention is a choice you must pin down).
**Skill level:** Intermediate–Advanced — the whole difficulty is the `1/N` / `√N` convention and the `fftshift` bookkeeping.

**Setup — confirm before you start:**

- [ ] `qmtoolkit/probability.py` from Chapter 2 (for `dx`-weighted norms) and `complex_tools.py` from Chapter 1.
...
```
Add a discrete Fourier-transform module to qmtoolkit. Vol. I Ch. 8 builds wave packets by
superposing plane waves; Vol. III Ch. 8 (Born approximation) is the FT of the potential.
The transform is the same operation both times.

Create qmtoolkit/fft_tools.py with:
  - ft(psi, x): forward transform returning (k, psi_k) on a properly-centered k-grid, using
    the symmetric-norm convention so that discrete Parseval holds; document the exact 1/N or
    dx/sqrt(2π) factor you use in the docstring — do NOT silently absorb it.
  - parseval_ratio(psi, x): return (∫|psi|^2 dx) / (∫|psi_k|^2 dk); should be 1.
Use np.fft.fft / fftfreq / fftshift; center the grids.

Create tests/test_fft_tools.py checking closed-form facts from this chapter:
  1. The FT of a Gaussian is a Gaussian: transform ψ(x)=exp(-x^2/(2a^2)) with a=1.0 on
     x=linspace(-16,16,2048); fit or compare |psi_k| to the analytic Gaussian of width 1/a
     and assert max relative deviation < 1e-3.
  2. Parseval/Plancherel: parseval_ratio ≈ 1 to 1e-10 for that ψ.
  3. Minimum-uncertainty: measure Δx from |ψ|^2 and Δk from |psi_k|^2 and assert
     Δx·Δk ≈ 0.5 to 2e-2 (the Gaussian saturates the bandwidth bound).

Then RUN `pytest -q`. Print the Parseval ratio, the Gaussian-fit deviation, and Δx·Δk with
PASS/FAIL. Do not touch unrelated files. Stop after the pytest summary and numbers.
```

**Expected output:** `qmtoolkit/fft_tools.py` and `tests/test_fft_tools.py` added; `pytest` passes; printed `Parseval ratio = 1.0000000000 PASS`, `Gaussian max rel dev = 4e-4 PASS`, `Δx·Δk = 0.5001 PASS`.

**The golden check (what makes this routine trustworthy):** three exact facts from this chapter — the Fourier transform of a Gaussian is a Gaussian (of reciprocal width), Parseval/Plancherel `∫|ψ|²dx = ∫|ψ̃|²dk` holds to `1e-10`, and the Gaussian saturates the bandwidth bound `Δx·Δk = 1/2`.

**What to inspect:** the Parseval ratio must be `1.0`, not `2π`, `1/2π`, or `N` — any of those is the fingerprint of the wrong FFT normalization; the transformed Gaussian's width should be `1/a` (halving `a` should double the k-space width); `Δx·Δk` should sit at ≈0.5, not 1 or 0.25.

**If it goes wrong:** an unnormalized `np.fft.fft` breaks Parseval by a factor of `N` (or `2π`) — fix the `1/N` / `dx·√(2π)` convention and state it in the docstring rather than absorbing it. If the k-space Gaussian is off-center or split at the array edges, you forgot `fftshift`/`fftfreq` centering.

**CLAUDE.md note:** Already covered by the project rule (the FFT `1/N` convention is the exact case the rule names).
