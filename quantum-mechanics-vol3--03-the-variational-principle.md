...
**Project:** Model a Real Quantum System · **Module this chapter adds:** a variational solver that minimizes ⟨H⟩ over a trial family and enforces the one-sided upper-bound guarantee — the volume's one method with no ε, only a bound.

**Tool:** Claude Code (default; Codex CLI or Cowork work identically — the scipy.optimize call is the same everywhere).
**Skill level:** Intermediate — a one-parameter minimization, but the discipline is in checking the bound, not the fit.
...
```
Add methods/variational.py to the qm-real-system repo. Import epsilon.py, validate.py,
report.py. Do not modify earlier chapters' modules.

Physics: helium ground state with the single-parameter trial psi ~ exp(-Zt (r1+r2)/a0),
whose energy in Hartree is E_V(Zt) = Zt^2 - 2*Z*Zt + (5/8)*Zt with Z=2. 1 Hartree = 27.211 eV.

1. FIRST handle the small parameter honestly. The variational method has NO breakdown small
   parameter; its guarantee is E_V >= E0_exact. Call
   epsilon.estimate_epsilon("variational", float('nan'), "N/A — bound only, not a small parameter")
   and print a note that the controlling diagnostic is instead the missing correlation energy,
   estimated as (E_V - E_measured)/E_measured after the fact.

2. THEN minimize E_V(Zt) over Zt with scipy.optimize (or analytically: Zt* = Z - 5/16).
   Confirm Zt* = 27/16 = 1.6875 and convert E_V(Zt*) to eV (expect about -77.5 eV).

3. THEN RUN the validation against the measured helium ground-state energy. I will paste the
   measured value (about -79.0 eV — look it up and cite author/year; do NOT invent it).
   - Assert the BOUND first: E_V >= E_measured (i.e. E_V is above the true energy). If E_V comes
     out below the measured energy, STOP and flag a normalization or sign bug — do not proceed.
   - Then call validate.check("He ground state", E_V, E_measured, 0.03, " eV").
   Print a five_move_report; the breakdown line names electron-electron correlation (the missing
   r12 cusp) as the dominant omitted effect and notes first-order PT gives -74.8 eV (worse).

4. STOP and show me methods/variational.py, the output, and the measured datum + citation.
   Delete nothing.
```

**Expected output:** `methods/variational.py`; `Zt* = 1.6875`; `E_V ≈ -77.5 eV`; an assertion that the bound `E_V ≥ E_measured` holds; and `He ground state: predicted≈-77.5 eV measured≈-79.0 eV err≈1.9% PASS` with a five-move report.

**The golden check (what makes this simulation trustworthy):** the variational energy must be **−77.5 eV, and it must lie above the measured −79.0 eV** (cite the measured helium ground-state energy you find; do not accept a fabricated value). That inequality is a theorem — any trial state gives an energy ≥ the true ground state — so a result below −79.0 eV is a guaranteed bug, not a better answer. The 1.9% error with a *single* free parameter is the point of the chapter.

**What to inspect:** does the bound stay above the true energy (E_V ≥ −79.0 eV)? Does Z̃* come out as exactly 27/16, and does the screening interpretation (each electron sees 2 − 5/16 of the nuclear charge) hold? Is the variational result closer to experiment than the −74.8 eV first-order-PT number quoted in the chapter?

**If it goes wrong:** a "variational energy" below −79.0 eV means a sign error in the kinetic term or a normalization mistake in the trial state — the bound cannot be violated, so trust the theorem and hunt the bug rather than celebrating the low number. If E_V comes out positive or wildly off, you likely mixed Hartree and eV — the E_V(Z̃) formula is in Hartree and must be converted once, at the end.

**CLAUDE.md note:** Add this method-specific line under the project rule: "The variational method reports no small parameter — its guarantee is the upper bound E_V ≥ E₀. Any computed energy below a trusted lower reference is a bug to surface, never a result to keep."
