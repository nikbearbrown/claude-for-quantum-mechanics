...
Individual atomic ions — typically $^{40}\text{Ca}^+$ or $^{171}\text{Yb}^+$ — are confined in Paul traps by oscillating electric fields. Two qubit encodings are used. Hyperfine qubits encode in two ground-state hyperfine levels connected by a microwave transition (e.g., $\nu \approx 12.6$ GHz for $^{171}\text{Yb}^+$); coherence times in seconds to minutes. Optical qubits encode in the ground state and a metastable excited state connected by a narrow optical transition; coherence times limited by the metastable lifetime and laser linewidth.
...
Neutral atoms — $^{87}\text{Rb}$, $^{133}\text{Cs}$, $^{171}\text{Yb}$, or $^{87}\text{Sr}$ — trapped in tightly focused laser beams (tweezers, $\sim1\,\mu\text{m}$ waist). Arrays generated by spatial light modulators or acousto-optic deflectors give arbitrary programmable geometries with hundreds to thousands of atoms. Qubits encoded in two hyperfine ground states.

Entangling gates via Rydberg blockade: two atoms are excited to high principal quantum number states ($n \sim 50$–100) that have large electric dipole moments. If one atom is in the Rydberg state, a second nearby atom cannot also be excited (the interaction energy shifts the double-excitation off resonance). This conditional excitation implements a CZ gate in $\sim0.1$–$1\,\mu\text{s}$. Atoms can be physically moved between gate operations, giving reconfigurable connectivity.
...
A nitrogen-vacancy (NV) center is a point defect: a substitutional nitrogen adjacent to a lattice vacancy. The $\text{NV}^-$ charge state has a spin-1 ground state with a zero-field splitting $D \approx 2.87$ GHz between $m_s = 0$ and $m_s = \pm1$ sublevels. An applied magnetic field $B$ further splits $m_s = \pm1$ by the Zeeman effect. The qubit is encoded in the $\{|m_s = 0\rangle, |m_s = -1\rangle\}$ subspace.
...
**Current state (mid-2026).** $T_1 \sim 1$–6 ms, $T_2 \sim 100\,\mu\text{s}$–2 ms with dynamical decoupling. Single-qubit fidelity $\sim99\%$.
...
**Project:** Reconstruct a Research Result · **Module this chapter adds:** `qrecon/hardware.py` — the NV-center spin-1 Hamiltonian and its ODMR transition frequencies f± = D ± (gμ_B/h)B, plus the two-level reduction check: the tool for reconstructing an NV magnetometry or Bell-test paper's spectrum from the applied field.

**Tool:** Claude Code (default; Codex CLI or Cowork work identically — diagonalizing a 3×3 real symmetric matrix is trivial on every platform; keep the gyromagnetic constant in MHz/mT throughout).
**Skill level:** Intermediate — you build a spin-1 Hamiltonian, diagonalize it, and must keep unit bookkeeping (GHz, MHz, mT) exact.

**Setup — confirm before you start:**

- [ ] `qrecon/core.py` exists; `qrecon/density.py` (Chapter 1) is available for the two-level check.
- [ ] Python ≥3.10 with numpy.
- [ ] The standing project rule is in `CLAUDE.md`.

**The Task — paste into your CLI:**
```
Add a hardware module to my qrecon toolkit. Python 3 + numpy. Read qrecon/core.py
first; do not rewrite shared helpers. Create qrecon/hardware.py and
tests/test_hardware.py only. Concept: the NV-center spin-1 ground-state Hamiltonian
H = D*Sz^2 + (g*muB/h)*B*Sz, and its two ODMR transition frequencies from m_s=0.

Use D = 2870 MHz (zero-field splitting) and gamma = 28.025 MHz/mT (g*muB/h).
In qrecon/hardware.py implement:
  - spin1_Sz(): the 3x3 spin-1 Sz operator with eigenvalues {+1,0,-1}.
  - nv_hamiltonian(B_mT): the 3x3 matrix D*Sz^2 + gamma*B*Sz (in MHz).
  - nv_odmr(B_mT): diagonalize H, return the two transition frequencies from the
    m_s=0 level: f_plus = D + gamma*B and f_minus = D - gamma*B (MHz).
  - splitting(B_mT): f_plus - f_minus = 2*gamma*B.

In tests/test_hardware.py verify, printing numbers, this chapter's analytic facts:
  1. At B=0: both transitions degenerate at D=2870 MHz.
  2. At B=30 mT: f_plus = 3.71 GHz and f_minus = 2.03 GHz within 1 MHz
     (2870 + 28.025*30 = 3710.75 MHz; 2870 - 840.75 = 2029.25 MHz).
  3. Linearity: splitting(B) = 56.05*B MHz/mT exactly (slope = 2*gamma).
  4. The transition frequencies from diagonalizing nv_hamiltonian match the closed-form
     D +/- gamma*B within 1e-6 MHz (numerics agree with the analytic formula).

Keep every frequency in MHz and every field in mT -- if a number is off by ~1000,
STOP and check a GHz/MHz mixup rather than rescaling to hide it.
When done, RUN the tests and print one line:
  "GOLDEN CHECK: f_plus(30mT)=<...>MHz f_minus(30mT)=<...>MHz slope=<...>MHz/mT diag_vs_formula=<...> -> PASS/FAIL"
PASS requires f_plus~3710.75, f_minus~2029.25, slope~56.05, diag_vs_formula<1e-6.
Show me the line, then STOP.
```

**Expected output:** `qrecon/hardware.py`, `tests/test_hardware.py`, and a line like `GOLDEN CHECK: f_plus(30mT)=3710.75MHz f_minus(30mT)=2029.25MHz slope=56.05MHz/mT diag_vs_formula=3.6e-13 -> PASS`.

**The golden check (what makes this simulation trustworthy):** the NV ODMR transitions are f± = D ± (gμ_B/h)B with D = 2.87 GHz and gμ_B/h = 28.025 MHz/mT, so at B = 30 mT they sit at 3.71 GHz and 2.03 GHz; the splitting Δf = 56.05·B MHz/mT is exactly linear in the axial field (the first-order Zeeman result); and numerical diagonalization of the spin-1 Hamiltonian must reproduce the closed-form frequencies.

**What to inspect:** does the zero-field spectrum show a single line at D (both m_s = ±1 degenerate)? Is the splitting slope exactly twice the gyromagnetic ratio? Do the diagonalized eigenvalue differences match D ± γB to machine precision?

**If it goes wrong:** a frequency off by ~1000× is a GHz-vs-MHz unit slip — the tripwire is the analytic-vs-diagonalization comparison in check 4. A splitting slope of 28 instead of 56 means you reported one transition frequency's shift instead of the gap between the two.

**CLAUDE.md note:** Already covered by the project rule — the closed-form f± = D ± 28B is the analytic identity the diagonalization is checked against.
