...
**Project:** Reconstruct a Research Result · **Module this chapter adds:** `reconstruct.py` — the integration script that imports every prior `qrecon` module, recomputes one real paper's central number from first principles, and emits the reconstruction dossier (recomputed number vs. the paper's reported number, plus your honesty verdict).

**Tool:** Claude Code (default; Codex CLI or Cowork work identically — the one difference is that in Cowork you can keep the dossier open as a live artifact while the recomputation runs, whereas Claude Code and Codex write it to a Markdown file).
**Skill level:** Advanced — this stitches the whole volume's toolkit together and, crucially, requires you to hold the line between what the code computes and what only you (having read the paper) may assert.

**Setup — confirm before you start:**

- [ ] All prior modules exist: `qrecon/core.py`, `density.py`, `entanglement.py`, `chsh.py`, `gates.py`, `teleport.py`, `lindblad.py`, `measurement.py`, `hardware.py`, `qec.py`.
- [ ] Python ≥3.10 with numpy.
- [ ] You have chosen ONE real paper and have it open, and the standing project rule is in `CLAUDE.md`.

**The Task — paste into your CLI:**
```
Build the capstone reconstruction script for my "Reconstruct a Research Result" project.
Python 3 + numpy. Import my existing qrecon modules (do NOT rewrite any of them):
chsh for a Bell test, qec for a QEC milestone, hardware for an NV sensing paper.
Create reconstruct.py and dossier_TEMPLATE.md only; touch nothing in qrecon/.

I will pick ONE doorway. reconstruct.py must:
  1. Ask (via a CONFIG dict at the top I fill in) which doorway: "bell", "qec", or "nv",
     and the paper's citation string plus its REPORTED central number, which I type in.
     If I leave the reported number blank, print "REPORTED = [read from paper and cite]"
     -- never guess or fill it.
  2. Recompute the central number from first principles using the matching module:
       bell -> chsh.chsh_S(|Phi+>, optimal angles) : expect S = 2*sqrt(2) = 2.828 (ideal),
               and print the local-realistic bound 2 for contrast.
       qec  -> qec.p_logical(p,d) and qec.suppression(p) : print the scaling and Lambda.
       nv   -> hardware.nv_odmr(B) : print f_plus, f_minus for the paper's stated field.
  3. Emit a dossier to dossier_TEMPLATE.md with sections:
       Recomputed number | Reported number (from paper) | Analytic bound/identity |
       Which tools/chapters used | [AUTHOR VERDICT: milestone vs marketing] (LEAVE BLANK).
     Leave the [AUTHOR VERDICT] line empty for me -- do not write the verdict yourself.
  4. End by RUNNING the recomputation for doorway "bell" as a self-test and printing:
     the recomputed S, the analytic bound, and PASS/FAIL against S = 2*sqrt(2).

Do not assert my recomputed number "agrees with" or "reproduces" the paper -- print
both numbers side by side and stop. Do not invent the reported value, error bars, or
the DOI. Do not fill the [AUTHOR VERDICT] line.
When done, print one line:
  "GOLDEN CHECK: recomputed_S=<...> analytic_bound=2*sqrt2=2.8284 lhv_bound=2 -> PASS/FAIL"
PASS requires recomputed_S within 1e-6 of 2.828427.
Show me the line and the dossier skeleton, then STOP.
```

**Expected output:** `reconstruct.py`, `dossier_TEMPLATE.md` (with the reported-number and [AUTHOR VERDICT] fields left for you), and a line like `GOLDEN CHECK: recomputed_S=2.828427 analytic_bound=2*sqrt2=2.8284 lhv_bound=2 -> PASS`.

**The golden check (what makes this simulation trustworthy):** the self-test recomputes the ideal Bell-test value S = 2√2 ≈ 2.828 against the local-realistic bound of 2 — a demonstration-of-physical-principle that no classical-simulation improvement can erode. For the QEC doorway the anchor is the scaling law Λ ≈ p_th/p; for the NV doorway it is f± = D ± 28B. In every case the tool prints the *recomputed* number and the *analytic* bound; the paper's reported number is entered by you from the paper and cited, never generated.

**What to inspect:** did the script leave the reported number and the [AUTHOR VERDICT] line blank for you to fill? Does the self-test hit 2√2 exactly? Does the dossier cite which chapters/tools each number came from? Is the recomputed number presented *beside* the reported one, with no claim that they "agree"?

**If it goes wrong:** if the script writes a milestone-vs-marketing verdict on its own, or fills in the paper's S/p_L/f± without your input, that violates the project rule — reject it and re-run insisting those fields stay empty. If the self-test S is not 2√2, the CHSH module's angles are being passed in degrees where radians are expected (Chapter 3's tripwire).

**CLAUDE.md note:** Already covered by the project rule; make sure the `[AUTHOR VERDICT]` line and the reported-number field are treated as author-only — the assistant recomputes and reports, the human adjudicates milestone vs. marketing.
