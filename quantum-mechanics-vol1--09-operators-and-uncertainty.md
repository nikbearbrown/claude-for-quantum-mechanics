...
**Project:** The 1D Quantum Sandbox · **Module this chapter adds:** the observables panel — numerical ⟨x⟩, ⟨p⟩, ⟨x²⟩, ⟨p²⟩, σx, σp and the σx·σp/(ℏ/2) readout for any state on the grid, using p̂ = −iℏ∂ₓ.

**Tool:** Claude Code (default; Codex CLI or Cowork work identically — in Cowork, open the panel in the built-in preview; the check runs under `node`).
**Skill level:** Intermediate — the momentum operator's finite-difference form and its load-bearing minus sign.

**Setup — confirm before you start:**

- [ ] `src/wavefn.js` (Chapter 3), `src/eigensolver.js` (Chapter 5), `src/grid.js` (Chapter 2), `src/constants.js` (Chapter 0) exist and pass their checks.
- [ ] Node ≥ 18 (plain ES modules); a browser to view the panel.
- [ ] The standing rule is in `CLAUDE.md`.

**The Task — paste into your CLI:**

```
Read src/wavefn.js, src/eigensolver.js, src/grid.js, src/constants.js (do NOT modify them). Create ONLY the new files below; leave all other files alone.

1. src/observables.js, exporting (all using sum_j ... * h):
   - expX(psi, grid), expX2(psi, grid) from |psi|^2.
   - expP(psi, grid), expP2(psi, grid) using p-hat = -i*hbar*d/dx by central difference; expP = Re[ sum_j conj(psi_j) (-i hbar (psi_{j+1}-psi_{j-1})/(2h)) h ]. The minus sign is load-bearing (Chapter 9).
   - sigmaX, sigmaP, and uncertaintyRatio(psi, grid) = sigmaX*sigmaP / (hbar/2).
   Add a D3 panel to index.html showing these numbers for the current state.

2. test/check-observables.mjs that:
   - for the infinite-well ground state (L = 10 nm, n = 1, from solveBound), asserts uncertaintyRatio ~= 1.136 to < 1% (Chapter 9 worked value 0.568 hbar / (hbar/2)).
   - for the minimum-uncertainty Gaussian, asserts uncertaintyRatio ~= 1.000 to < 1%.
   - asserts uncertaintyRatio >= 1 (within -1e-6) for well modes n = 1..5 (Robertson bound).
   - asserts a rightward Gaussian (k0 > 0) has expP > 0 and a leftward one (k0 < 0) has expP < 0.
   - prints PASS/FAIL with the numbers.

Then RUN `node test/check-observables.mjs` and show the output. Stop.
```

**Expected output:** `src/observables.js`, an observables panel in `index.html`, and printed `PASS ISW n=1 ratio = 1.136`, `PASS Gaussian ratio = 1.000`, `PASS ratio ≥ 1 for n=1..5`, `PASS ⟨p⟩ sign tracks k₀`.

**The golden check (what makes this simulation trustworthy):** for the infinite-well ground state σx·σp/(ℏ/2) ≈ 1.136 (analytic 0.568ℏ), for the minimum-uncertainty Gaussian the ratio is 1.000, and σx·σp ≥ ℏ/2 for every state (Robertson) — all exact results derived in this chapter.

**What to inspect:** that ⟨p⟩ = 0 for a real standing wave but > 0 for a rightward packet; that the Gaussian sits on the bound while the well state sits above it; that the ratio grows with mode number n.

**If it goes wrong:** using +iℏ∂ₓ (the wrong sign) flips ⟨p⟩ for a moving packet while leaving the σx·σp product unchanged — so the uncertainty checks pass but the direction of motion is inverted. The k₀ > 0 ⇒ ⟨p⟩ > 0 assertion is there precisely to catch this; do not "flip a sign to make the arrow point right" without surfacing it.
