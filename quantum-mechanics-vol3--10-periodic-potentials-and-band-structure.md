...
**Project:** Model a Real Quantum System · **Module this chapter adds:** a band-structure module (Kronig–Penney dispersion and gaps) validated against the nearly-free-electron gap = 2|V_{G₁}| in the weak-potential limit.

**Tool:** Claude Code (default; Codex CLI or Cowork work identically — root-finding and array sweeps in numpy).
**Skill level:** Intermediate — a transcendental dispersion relation and one perturbative limit to check against.

**Setup — confirm before you start:**

- [ ] The `qm-real-system/` repo, with `epsilon.py`, `validate.py`, `report.py` from Chapter 0.
...
```
Add methods/bands.py to the qm-real-system repo. Import epsilon.py, validate.py, report.py.
Do not modify earlier chapters' modules.

Physics: delta-comb Kronig-Penney model. Dispersion (natural units hbar=2m=a=1, energy in
hbar^2/2ma^2, so E = (alpha a)^2):
  cos(k a) = f(alpha a) = cos(alpha a) + (P/(alpha a)) sin(alpha a).
Allowed bands are where |f| <= 1; gaps are where |f| > 1. The first gap opens at the zone
boundary alpha a = pi.

1. FIRST estimate the small parameter for the nearly-free-electron (NFE) comparison: the lattice
   is a weak perturbation when the relevant Fourier coefficient is small vs the zone-boundary
   kinetic energy. For the delta comb, the first Fourier component gives an NFE gap prediction
   ~ 2 P / pi (in these units at the first boundary). Use epsilon = P / pi^2 as the weak-lattice
   ratio; call epsilon.estimate_epsilon("NFE (weak lattice)", P/pi**2, "epsilon = P/pi^2 >~ 0.3").

2. THEN implement:
   - f(alpha_a, P).
   - first_gap(P): sweep alpha_a from 0 to 2*pi finely; find the band edges bracketing the
     zone boundary alpha_a = pi (where |f| first exceeds 1 above pi, and re-enters below);
     return (E_lower_edge, E_upper_edge) and gap = E_upper - E_lower in these units.

3. THEN RUN the verification for a WEAK lattice (small P, e.g. P = 0.2):
   - Numerically extract first_gap(P).
   - Compare to the NFE prediction gap_NFE = 2 * |V_G1|, where for the delta comb the first
     Fourier amplitude gives gap_NFE ~ 2 P / pi in these units.
   - validate.check("KP gap vs NFE (weak)", gap_numeric, gap_NFE, 0.10, "").
   Then sweep P in {0.2, 1, 5, 15} and print gap and epsilon for each, showing the gap widen and
   the NFE prediction diverge from the exact gap as epsilon grows. Print a five_move_report;
   breakdown line notes the 1D idealization and neglect of electron-electron interaction.

STOP and show me methods/bands.py and the run output. Delete nothing.
```

**Expected output:** `methods/bands.py`; `epsilon[NFE (weak lattice)] ≈ 0.02` at P=0.2; a numerically extracted first gap; `KP gap vs NFE (weak): ... err<10% PASS`; and a P-sweep showing the gap growing while the NFE estimate drifts off as ε grows.

**The golden check (what makes this simulation trustworthy):** for a weak lattice the exact Kronig–Penney first band gap must match the nearly-free-electron prediction **gap = 2|V_{G₁}|** (≈ 2P/π in these units) to within ~10%, confirming that the gap equals twice the relevant Fourier component of the potential. As P grows (ε = P/π² increases), the two must diverge — the perturbative NFE result is only valid for a weak potential.

**What to inspect:** at small P, is ε ≪ 1 and does the numeric gap track 2|V_{G₁}|? Does the gap open exactly at the zone boundary αa = π (where f(π) = −1)? As P → large, does the gap widen and the band narrow toward the isolated-atom limit, with the NFE estimate visibly failing?

**If it goes wrong:** if the numeric gap and NFE prediction disagree even at P = 0.2, your band-edge finder is catching the wrong |f| = 1 crossing — bracket specifically around αa = π. If the gap comes out negative or zero, the edge ordering (upper minus lower) is flipped; surface it. Comparing at P = 15 and calling the NFE mismatch a "bug" is the error — that is the weak-lattice approximation correctly breaking down, and the ε printout is what tells you so.

**CLAUDE.md note:** Already covered by the project rule (state ε = P/π², check the gap against the NFE 2|V_{G₁}| limit, name the breakdown as P grows).
