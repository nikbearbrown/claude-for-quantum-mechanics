...
**Project:** Build the Atom · **Module this chapter adds:** the screened hydrogenic radial functions R_nℓ(r; Z_eff) — analytic Laguerre-based orbitals, their energies −13.6·Z_eff²/n² eV, and radial expectation values. These are the shapes the multi-electron simulator places electrons into, scaled by the Z_eff that Slater's rules will supply in the capstone.

**Tool:** Claude Code (default; Codex CLI or Cowork work identically — uses `scipy.special.genlaguerre`, available in all three).
**Skill level:** Intermediate — the associated-Laguerre normalization is fiddly, but the golden checks catch a wrong constant immediately.

**Setup — confirm before you start:**
- [ ] `atom/constants.py` (Chapter 0) exists; `atom/radial.py` (Chapter 5) is available for a numerical cross-check if you want one.
...
```
Add the hydrogenic radial-function module. Read atom/constants.py to reuse a0 and the Rydberg energy; do
not modify it. Create ONLY atom/hydrogen.py and tests/test_hydrogen.py. Do not delete anything.

In atom/hydrogen.py implement, with docstrings, the screened hydrogenic orbitals from this chapter
(allow an effective charge Z_eff, default 1; lengths in units of a0):
  - R_nl(n, ell, r, Z=1)       -> normalized radial function using scipy.special.genlaguerre, degree
                                  n-ell-1, i.e. R ~ (2Z r/n)^ell exp(-Z r/n) L_{n-ell-1}^{2ell+1}(2Z r/n).
  - radial_pdf(n, ell, r, Z=1) -> P(r) = r^2 |R_nl|^2  (the shell-weighted density; the r^2 is mandatory)
  - energy_eV(n, Z=1)          -> -13.606 * Z**2 / n**2   (computed from the constants module, not typed)
  - mean_r(n, ell, Z=1)        -> <r> via numerical integral of r*P(r); compare to (a0/2Z)[3n^2-ell(ell+1)]

In tests/test_hydrogen.py write a runnable check (asserts + prints), Z=1 unless noted:
  (a) NORMALIZATION: print PASS if integral of radial_pdf over r in [0, 50 a0] == 1 to 1e-4 for
      (n,ell) in {(1,0),(2,0),(2,1),(3,2)};
  (b) SPECTRUM (golden check): print PASS if energy_eV(n) matches -13.606/n^2 eV to < 0.5% for n=1,2,3
      (i.e. -13.6, -3.40, -1.51 eV); print the three values;
  (c) GROUND-STATE RADII: print PASS if the most-probable radius of the 1s state (argmax of radial_pdf)
      is a0 to < 1% AND mean_r(1,0) == 1.5 a0 to < 0.5% -- the r_mp != <r> signature of a skewed shell
      distribution;
  (d) MEAN-R FORMULA: print PASS if mean_r(2,0) and mean_r(2,1) match (a0/2)[3n^2-ell(ell+1)] = 6a0 and
      5a0 to 0.5%.

Then RUN: python tests/test_hydrogen.py  and show me the PASS/FAIL lines. Stop after that.
```

**Expected output:** `atom/hydrogen.py`, `tests/test_hydrogen.py`, and printed lines such as `∫P dr=1.0000 for 4/4 orbitals -> PASS`, `E_n = −13.6,−3.40,−1.51 eV (<0.5%) -> PASS`, `r_mp(1s)=1.000 a0, ⟨r⟩(1s)=1.500 a0 -> PASS`, `⟨r⟩(2s)=6.0 a0, ⟨r⟩(2p)=5.0 a0 -> PASS`.

**The golden check (what makes this simulation trustworthy):** the bound-state energies come out E_n = −13.606/n² eV to under 0.5% (−13.6, −3.40, −1.51 for n=1,2,3), the 1s most-probable radius is the Bohr radius a₀ while the mean radius is 3a₀/2, and every R_nℓ integrates (with the r² shell factor) to 1. The r_mp ≠ ⟨r⟩ gap is the wave-mechanical fingerprint, reproduced numerically.

**What to inspect:** that the r² Jacobian is present in `radial_pdf` (without it P(0)≠0 and ⟨r⟩ is wrong); that the Z_eff scaling enters as Z² in energy and 1/Z in length; that ⟨r⟩ from the numerical integral matches the analytic (a₀/2Z)[3n²−ℓ(ℓ+1)].

**If it goes wrong:** the most common failure is a mis-normalized Laguerre orbital — `genlaguerre` degree or the 2Z/n argument scaling off by a factor — which breaks the normalization check while the energies (which don't depend on the norm) still look fine. Trust the ∫P dr = 1 check; fix the normalization constant, don't rescale P by hand to force unit area.
