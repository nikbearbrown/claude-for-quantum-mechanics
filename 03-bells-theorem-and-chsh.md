...
**Project:** Reconstruct a Research Result · **Module this chapter adds:** `qrecon/chsh.py` — the CHSH correlation function computed from first principles for an arbitrary two-qubit state and four measurement angles, plus a brute-force local-hidden-variable bound: the core tool for reconstructing a Bell-test paper's reported S.

**Tool:** Claude Code (default; Codex CLI or Cowork work identically — watch that the model uses radians internally regardless of which CLI you paste into).
**Skill level:** Intermediate — the physics is a one-line correlation formula, but you must build the measurement operators (a⃗·σ⃗)⊗(b⃗·σ⃗) correctly and keep degrees vs. radians straight.

**Setup — confirm before you start:**

- [ ] `qrecon/core.py` (kets, gates, tensor) and `qrecon/density.py` exist.
- [ ] Python ≥3.10 with numpy.
- [ ] The standing project rule is in `CLAUDE.md`.

**The Task — paste into your CLI:**
```
Add a CHSH module to my qrecon toolkit. Python 3 + numpy. Read qrecon/core.py first
and reuse its Pauli matrices and tensor(); do not rewrite them. Create qrecon/chsh.py
and tests/test_chsh.py only. Concept: the CHSH parameter and the Tsirelson bound.

In qrecon/chsh.py implement:
  - measure_op(theta): the single-qubit observable cos(theta)*Z + sin(theta)*X
    (measurement in the plane at angle theta from the z-axis). ACCEPT theta in RADIANS;
    provide a deg2rad helper but store internally in radians.
  - correlation(state, theta_a, theta_b): E = <state| M(theta_a) tensor M(theta_b) |state>.
  - chsh_S(state, a1, a2, b1, b2): S = E(a1,b1) + E(a1,b2) + E(a2,b1) - E(a2,b2).
  - lhv_max(a1, a2, b1, b2): brute-force max over all 16 deterministic +/-1 assignments
    A_i(lambda), B_j(lambda) of |A1 B1 + A1 B2 + A2 B1 - A2 B2| -> must be 2.

In tests/test_chsh.py verify, printing numbers, this chapter's analytic facts:
  1. For |Phi+>=(|00>+|11>)/sqrt2 at the optimal angles (0, 90, 45, -45) degrees,
     S = 2*sqrt(2) = 2.8284 within 1e-9.
  2. For the SAME angles, lhv_max = 2 exactly (the local-realistic bound).
  3. Tsirelson ceiling: draw 10000 random angle quadruples; assert every |S| for
     |Phi+> stays <= 2*sqrt(2)+1e-9 (no quantum state beats 2.828).
  4. The common-error case: all four angles equal -> S = 2 for |Phi+> (no violation).

Never convert an angle you were handed in degrees as if it were radians -- if a test
value looks off by the deg/rad factor, STOP and surface it rather than patching.
When done, RUN the tests and print one line:
  "GOLDEN CHECK: S_opt=<...> lhv_max=<...> S_maxover10000=<...> S_equalangles=<...> -> PASS/FAIL"
PASS requires S_opt=2.8284, lhv_max=2.0, S_maxover10000<=2.8285, S_equalangles=2.0.
Show me the line, then STOP.
```

**Expected output:** `qrecon/chsh.py`, `tests/test_chsh.py`, and a line like `GOLDEN CHECK: S_opt=2.828427 lhv_max=2.000000 S_maxover10000=2.828427 S_equalangles=2.000000 -> PASS`.

**The golden check (what makes this simulation trustworthy):** at the optimal angles the Bell state gives S = 2√2 ≈ 2.828 (the Tsirelson bound), any local-hidden-variable model is capped at |S| ≤ 2, and no quantum state exceeds 2√2 no matter what angles you scan. The gap 2√2 − 2 ≈ 0.828 is the entire content of a Bell test.

**What to inspect:** does S exceed 2 but never exceed 2√2? Does the brute-force LHV maximum come out to exactly 2? Does correlation(state, θ, θ) = +1 for |Φ⁺⟩ when both measure the same axis?

**If it goes wrong:** an S above 2.829 is the classic bug — angles passed in degrees but consumed as radians (or vice versa); the Tsirelson ceiling being violated is your tripwire. An S that never exceeds 2 usually means the measurement operator lost its X-component (you built cos·Z + sin·Z by a copy-paste slip).

**CLAUDE.md note:** Already covered by the project rule — the Tsirelson bound 2√2 is one of the named analytic checks.
