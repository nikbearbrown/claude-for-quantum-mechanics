...
**Project:** The 1D Quantum Sandbox · **Module this chapter adds:** the uniform spatial grid, auto-sized so its spacing resolves the de Broglie wavelength of the particle it will hold.

**Tool:** Claude Code (default; Codex CLI or Cowork work identically — no browser needed for this module; it is a pure numerical utility checked under `node`).
**Skill level:** Beginner–Intermediate — one array and one sizing rule, but the sizing rule is the whole point.

**Setup — confirm before you start:**

- [ ] `src/constants.js` (Chapter 0) exists and passes its check.
...
```
Read src/constants.js (do NOT modify it). Create ONLY these new files; touch nothing else.

1. src/grid.js, importing from ./constants.js, exporting:
   - makeGrid({ xMin, xMax, N }) -> { x: Float64Array(N), h }  where h = (xMax - xMin)/(N-1) and x[j] = xMin + j*h.
   - deBroglieWavelength(energy_eV, mass_kg) -> wavelength in nm, using lambda = h / sqrt(2*m*K) with K converted from eV to joules. This is Chapter 2's lambda = h/p.
   - gridForParticle(energy_eV, mass_kg, spanWavelengths, pointsPerWavelength) -> a grid whose spacing h <= lambda/pointsPerWavelength and whose width spans spanWavelengths de Broglie wavelengths.

2. test/check-grid.mjs that:
   - asserts deBroglieWavelength(54, m_e) = 0.167 nm to < 1% (the Davisson-Germer electron).
   - asserts the electron shortcut lambda ~= 1.226 nm / sqrt(V) at V = 54, 100, 400 volts (K = eV) to < 1%. Do NOT hard-code 0.167; compute it and compare to the looked-up shortcut.
   - builds gridForParticle(54, m_e, 20, 10) and asserts h <= lambda/10.
   - prints "PASS/FAIL lambda(54 eV e-) = ... nm", the shortcut table, and "PASS/FAIL grid resolves lambda (h = ... nm, lambda = ... nm)".

Then RUN `node test/check-grid.mjs` and show the output. Stop.
```

**Expected output:** `src/grid.js`, `test/check-grid.mjs`, and a printed `PASS λ(54 eV e⁻) = 0.167 nm` plus a shortcut table and `PASS grid resolves λ (h ≤ λ/10)`.

**The golden check (what makes this simulation trustworthy):** the de Broglie wavelength of a 54 eV electron is 0.167 nm (analytic, from λ = h/√(2mK)), and the electron shortcut λ ≈ 1.226 nm/√(V volts) is reproduced to < 1% at 54, 100, and 400 V — the parameter-free relation Davisson–Germer rests on.

**What to inspect:** that `h` prints in nm with ≥ 10 points per wavelength; that λ scales as 1/√V (quadruple V and λ halves); that the grid width covers many wavelengths, not one.

**If it goes wrong:** the classic error is leaving K in eV inside √(2mK) instead of converting to joules, which scales every wavelength by √(1.602×10⁻¹⁹) ≈ 4×10⁻¹⁰. The 0.167 nm anchor catches it instantly — a wavelength off by ten orders of magnitude means the eV→J conversion was skipped.
