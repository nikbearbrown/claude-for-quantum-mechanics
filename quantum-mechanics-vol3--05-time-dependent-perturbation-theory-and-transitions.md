...
**Project:** Model a Real Quantum System · **Module this chapter adds:** a time-dependent-PT / Rabi module that computes the first-order transition probability and the exact two-level Rabi solution, and locates where PT breaks (Ωt ≳ 1).

**Tool:** Claude Code (default; Codex CLI or Cowork work identically — plain numpy arithmetic).
**Skill level:** Intermediate — closed-form formulas, but the whole point is diagnosing where the approximation predicts a probability above 1.
...
```
Add methods/tdpt.py to the qm-real-system repo. Import epsilon.py, validate.py, report.py.
Do not modify earlier chapters' modules.

Physics: a two-level atom driven on resonance, coupling hbar*Omega. First-order PT gives
P_PT(t) = (Omega t / 2)^2; the exact RWA Rabi solution gives P_exact(t) = sin^2(Omega t / 2).
Off resonance (detuning Delta): P_exact = Omega^2/(Omega^2+Delta^2) * sin^2(sqrt(Omega^2+Delta^2) t / 2).

1. FIRST estimate the small parameter. TDPT is controlled by epsilon = Omega*t (coupling times
   elapsed time), NOT coupling alone. For a chosen small time t = 0.2/Omega, call
   epsilon.estimate_epsilon("TDPT", Omega*t, "epsilon = Omega*t >~ 1") and warn if large.

2. THEN implement P_pt(Omega, t), P_rabi(Omega, Delta, t). Confirm on resonance P_rabi reaches
   exactly 1 at the pi-pulse t_pi = pi/Omega, and off resonance the maximum is capped at
   Omega^2/(Omega^2+Delta^2) (< 1).

3. THEN RUN two verifications:
   (a) Agreement in-regime: at epsilon = Omega*t = 0.2, call
       validate.check("TDPT vs exact (small Omega t)", P_pt, P_rabi_resonant, 0.05, "").
   (b) Breakdown out-of-regime: at the pi-pulse (Omega*t = pi) assert P_pt = (pi/2)^2 ~= 2.47 > 1
       while P_exact = 1 — print this as an explicit "PT PREDICTS P>1: INVALID" line, not a PASS.
   Print a five_move_report; breakdown line notes depletion of the initial state (the return half
   of the Rabi cycle) is what PT misses.

STOP and show me methods/tdpt.py and the run output. Delete nothing.
```

**Expected output:** `methods/tdpt.py`; `epsilon[TDPT] = 0.2` at the small time; `TDPT vs exact (small Omega t): ... err<5% PASS`; and an explicit `PT PREDICTS P>1: INVALID` line at the π-pulse where P_PT ≈ 2.47 vs exact 1.0, with a five-move report.

**The golden check (what makes this simulation trustworthy):** for Ωt ≪ 1, first-order PT $(\Omega t/2)^2$ must agree with the exact Rabi $\sin^2(\Omega t/2)$ to within 5% (they share the same leading term). And the exact on-resonance probability must reach **exactly 1** at the π-pulse t = π/Ω, where PT instead predicts $(\pi/2)^2 ≈ 2.47$ — an unphysical probability above 1 that is the chapter's headline diagnostic of PT breakdown. Off resonance the exact maximum is capped at Ω²/(Ω²+Δ²) < 1.

**What to inspect:** is ε = Ωt (does halving Ω *and* holding t fixed halve ε)? Does the exact curve stay bounded in [0,1] while the PT parabola crosses 1? Does the off-resonance cap Ω²/(Ω²+Δ²) match a direct evaluation at Δ = Ω (should be ½) and Δ = 2Ω (should be 1/5)?

**If it goes wrong:** if PT and exact disagree even at Ωt = 0.1, check you used Ωt/2 (not Ωt) inside the sine — a factor-of-two slip. If your "exact" probability ever exceeds 1, you have dropped the Ω²/(Ω²+Δ²) prefactor or mis-signed the detuning; surface it. Treating Ωt ≈ π as if PT were valid is the error the chapter is built around — report P > 1 as invalidity, never clip it to 1.

**CLAUDE.md note:** Add this method-specific line under the project rule: "For time-dependent perturbation theory the small parameter is Ωt (coupling × time), not coupling alone; a first-order probability exceeding 1 is a breakdown to report, never a value to clip."
