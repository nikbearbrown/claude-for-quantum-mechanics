...
**Project:** Build the Atom · **Module this chapter adds:** the angular-momentum matrix generator — build J_z, J₊, J₋, J_x, J_y, and J² for any j (integer or half-integer) from the ladder normalization formula alone. This is the reusable factory for the ℓ,mℓ orbital labels and the 2ℓ+1 subshell multiplicity.

**Tool:** Claude Code (default; Codex CLI or Cowork work identically — small dense matrices, no environment differences).
**Skill level:** Intermediate — the ladder coefficients ℏ√[(ℓ−m)(ℓ+m+1)] and correct sub/super-diagonal placement are the whole game.

**Setup — confirm before you start:**
- [ ] `atom/commutators.py` (Chapter 3) exists and its tests pass (you'll reuse `commutator`).
- [ ] Python ≥3.10 with numpy.
- [ ] Already covered by the project rule (confirm `CLAUDE.md` carries it).

**The Task — paste into your CLI:**
```
Add the angular-momentum matrix generator. Read atom/commutators.py to reuse commutator; do not modify it.
Create ONLY atom/angmom.py and tests/test_angmom.py. Do not delete anything.

In atom/angmom.py implement, with docstrings (units hbar=1), a factory that works for any j in
{0, 1/2, 1, 3/2, 2, ...}:
  - jmat(j) -> dict with keys 'Jz','Jp','Jm','Jx','Jy','J2' as (2j+1)x(2j+1) numpy arrays,
    built ONLY from the normalization formula J_+|j,m> = sqrt((j-m)(j+m+1)) |j,m+1> (and its conjugate for
    J_-), with basis ordered m = -j, ..., +j. Then Jx=(Jp+Jm)/2, Jy=(Jp-Jm)/(2j... use 2i), J2 from
    Jm@Jp + Jz@Jz + Jz.
  - multiplicity(j) -> 2j+1 (the number of mℓ states in a subshell of that ℓ)

In tests/test_angmom.py write a runnable check (asserts + prints) for j = 1, 2, and 1/2:
  (a) ALGEBRA CLOSES: for each j print PASS if commutator(Jx,Jy) == 1j*Jz to 1e-12 (and cyclic);
  (b) CASIMIR (golden check): for each j print PASS if J2 == j*(j+1)*I to 1e-12 -- e.g. j=2 gives
      J2 = 6*I exactly; print the diagonal of J2;
  (c) TERMINATION + COUNT: print PASS if J_+ annihilates the top state |j,+j> (zero column) and J_-
      annihilates |j,-j>, AND multiplicity(j) == 2j+1 for each j (1->3, 2->5, 1/2->2).

Then RUN: python tests/test_angmom.py  and show me the PASS/FAIL lines. Stop after that.
```

**Expected output:** `atom/angmom.py`, `tests/test_angmom.py`, and printed lines such as `j=1,2,½: [Jx,Jy]=iJz -> PASS`, `J²=j(j+1)I: j=2 gives diag=[6,6,6,6,6] -> PASS`, `J₊|j,j⟩=0, J₋|j,−j⟩=0, multiplicities {3,5,2} -> PASS`.

**The golden check (what makes this simulation trustworthy):** for every j the constructed matrices satisfy [J_x,J_y] = iħJ_z and the Casimir J² = ħ²j(j+1)·I to machine precision — for j=2, J² is exactly 6ħ²·I on a 5-dimensional space (2ℓ+1 = 5). The raising/lowering operators annihilate the top/bottom rungs exactly, confirming the ladder terminates rather than running off.

**What to inspect:** that J² is a multiple of the identity (all diagonal entries equal j(j+1), all off-diagonal zero); the degeneracy count 2ℓ+1 (=3 for ℓ=1, =5 for ℓ=2) that becomes the subshell orbital capacity; that the factory works unchanged for j=½ and reproduces (ħ/2)σ_i.

**If it goes wrong:** the usual bug is J₊ on the super-diagonal vs sub-diagonal (an off-by-one or a transpose), which makes [J_x,J_y] come out as −iJ_z or makes J² non-diagonal. Fix by re-deriving from J₊|j,m⟩ = √[(j−m)(j+m+1)]|j,m+1⟩ with the basis ordered m = −j…+j; do not transpose blindly until the test passes.
