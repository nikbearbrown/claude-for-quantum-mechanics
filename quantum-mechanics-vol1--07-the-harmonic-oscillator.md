...
**Project:** The 1D Quantum Sandbox · **Module this chapter adds:** the harmonic-oscillator potential and a validation harness that runs the Chapter 5 eigensolver on V = ½mω²x² and confirms the equally spaced spectrum.

**Tool:** Claude Code (default; Codex CLI or Cowork work identically — reuses the eigensolver and `mathjs` already installed; check runs under `node`).
**Skill level:** Intermediate — no new solver, but the grid must be wide enough or the high states are wrong.

**Setup — confirm before you start:**

- [ ] `src/eigensolver.js` (Chapter 5), `src/grid.js` (Chapter 2), `src/constants.js` (Chapter 0) exist and pass their checks.
- [ ] Node ≥ 18 (plain ES modules); `mathjs` installed.
- [ ] The standing rule is in `CLAUDE.md`.

**The Task — paste into your CLI:**

```
Read src/eigensolver.js, src/grid.js, src/constants.js (do NOT modify them). Create ONLY the new files below; leave all other files alone.

1. src/harmonic.js, exporting:
   - harmonicV(grid, mass, omega) -> Float64Array of 0.5 * mass * omega^2 * x_j^2 (in joules), for use as Varray.
   - validateHO(omega, mass, nEig) that: computes x0 = sqrt(hbar/(mass*omega)); builds a grid spanning at least +/- 6*x0 with enough points; calls solveBound with harmonicV; returns { energies_eV, spacings_eV }.

2. test/check-harmonic.mjs with omega = 1e14 rad/s, mass = m_e, nEig = 8:
   - assert (E_n - E_0)/(hbar*omega) ~= n to < 1% for n = 1..7 (Chapter 7: E_n = (n+1/2) hbar omega).
   - assert E_0 / (hbar*omega) = 0.5 to < 1% (zero-point energy).
   - assert consecutive gaps E_{n+1} - E_n are all equal to < 1% (the evenly spaced "ruler").
   - assert the ground state has 0 nodes and its sigma_x*sigma_p / (hbar/2) ~= 1 to < 2% (Kennard saturated).
   - print the energy/spacing table and PASS/FAIL lines.

Then RUN `node test/check-harmonic.mjs` and show the output. Stop.
```

**Expected output:** `src/harmonic.js`, `test/check-harmonic.mjs`, and a printed table showing E₀ = ½ℏω, uniform gaps ℏω, and `PASS` on spacing, zero-point energy, and the ground-state uncertainty product.

**The golden check (what makes this simulation trustworthy):** Eₙ = (n + ½)ℏω — the level spacing is uniform at ΔE = ℏω to < 1%, the ground state sits at E₀ = ℏω/2, and the Gaussian ground state saturates σx·σp = ℏ/2 to < 2%. These are the exact ladder-operator results of this chapter.

**What to inspect:** that the gaps form a ruler (all equal); that E₀ is ½ℏω and not zero; that the ground state has no nodes and is a Gaussian.

**If it goes wrong:** the dominant failure is a grid narrower than about ±5x₀ (x₀ = √(ℏ/mω)), which truncates the wave functions at the wall and biases the eigenvalues high — the spacing then drifts upward for the higher states instead of staying flat. Widen the grid until the highest state you keep decays to the boundary; do not accept a spacing that grows with n.
