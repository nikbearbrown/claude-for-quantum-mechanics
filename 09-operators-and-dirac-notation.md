...
**Project:** The QM Math Toolkit · **Module this chapter adds:** `operators.py` — Dirac-notation operator utilities (adjoint, commutator, expectation value, outer product) that enforce type discipline and the real-expectation-value reflex, the bra-ket language of II·1–II·3.

**Tool:** Claude Code (default; Codex CLI or Cowork work identically — no difference here).
**Skill level:** Intermediate — the payoff is machine-checking identities (`[σ_x,σ_y]=2iσ_z`, `⟨A⟩` real) that this chapter proves by hand.

**Setup — confirm before you start:**

- [ ] `qmtoolkit/eigen.py` from Chapter 8 and `inner_product.py` from Chapter 7.
...
```
Add Dirac-notation operator tools to qmtoolkit. Vol. II Chs. 1–3 (state space, observables,
time evolution) are written in bra-ket algebra: adjoints, commutators, expectation values.

Create qmtoolkit/operators.py with:
  - dagger(A): the Hermitian conjugate A.conj().T.
  - commutator(A, B): A@B - B@A.
  - expval(A, psi): <psi|A|psi> = conj(psi) @ A @ psi for a normalized column vector psi.
  - outer(psi, phi): |psi><phi| = np.outer(psi, phi.conj()).
Define the Pauli matrices sx, sy, sz in the module.
Leave other files untouched.

Create tests/test_operators.py checking closed-form facts from this chapter:
  1. (A@B).dagger == B.dagger @ A.dagger for random complex A,B to 1e-13 (order reversal).
  2. commutator(sx, sy) equals 2j*sz exactly (to 1e-14); likewise [sy,sz]=2j*sx, [sz,sx]=2j*sy.
  3. For a Hermitian A and any normalized psi, expval(A, psi) has |Im| < 1e-13 — a Hermitian
     operator has a REAL expectation value; a complex result signals a dropped conjugate.
  4. outer(e_i, e_j) reproduces the matrix unit E_ij for standard basis vectors.

Then RUN `pytest -q`. Print the adjoint-reversal residual, the three Pauli commutators
(as sz/sx/sy multiples), and max|Im expval| over random states, with PASS/FAIL. Do not
touch unrelated files. Stop after the pytest summary and numbers.
```

**Expected output:** `qmtoolkit/operators.py` and `tests/test_operators.py` added; `pytest` passes; printed `(AB)† − B†A† = 6e-16 PASS`, `[σx,σy] = 2i·σz PASS`, `max|Im⟨A⟩| = 1e-17 PASS`.

**The golden check (what makes this routine trustworthy):** three exact facts from this chapter — the adjoint reverses order `(AB)† = B†A†` to `1e-13`; the Pauli algebra `[σ_x,σ_y] = 2iσ_z` (and cyclic) holds exactly; and the expectation value of a Hermitian operator is real, `Im⟨A⟩ < 1e-13`, for every normalized state.

**What to inspect:** that `[σ_x,σ_y]` returns `2iσ_z` and not `iσ_z` (a factor-of-2 convention slip) or `−2iσ_z` (a sign/order error); that `Im⟨A⟩` is at the rounding floor across many random states, not just one; that `outer` conjugates the bra (`np.outer(psi, phi.conj())`).

**If it goes wrong:** a nonzero `Im⟨A⟩` for a Hermitian operator means `expval` used `psi @ A @ psi` without conjugating the bra — the exact reflex diagnostic this chapter names. If the commutator is off by a factor of 2, check whether spin generators `S_i = (ℏ/2)σ_i` were mixed in; keep the Pauli convention `[σ_i,σ_j] = 2iε_{ijk}σ_k`.
