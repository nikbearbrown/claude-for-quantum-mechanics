...
![Boson and fermion classification by constituent fermion count](../images/10-identical-particles-fig-01.png)
*Figure 10.1 — Taxonomy: particles with an even number of constituent fermions are bosons (left branch, blue) while those with an odd count are fermions (right branch, orange); terminus icons show macroscopic ground-state pile-up for bosons and the Pauli exclusion stack for fermions.*
...
![2×2 Slater determinant row-swap and sign flip](../images/10-identical-particles-fig-02.png)
*Figure 10.2 — Swapping the two particle labels (rows) in a $2\times2$ Slater determinant flips the sign (center panel); when both rows are identical, the determinant is identically zero — the Pauli exclusion principle as a theorem about determinants.*
...
![N×N Slater determinant structure](../images/10-identical-particles-fig-03.png)
*Figure 10.3 — The $N\times N$ Slater determinant: diagonal cells (particle $i$ in orbital $i$, highlighted) and off-diagonal cells; any row swap flips the sign, and two identical columns produce an identically zero determinant.*
...
![Para/ortho helium energy splitting: J and K integrals](../images/10-identical-particles-fig-04.png)
*Figure 10.4 — Energy-level diagram for the helium 1s2s configuration: the parahelium singlet (orange, higher) lies above the orthohelium triplet (green, lower) by $2K \approx 0.80$ eV; brackets indicate the direct integral $J$ and exchange integral $K$ contributions.*
...
![Spatial symmetry to Coulomb energy: three-step mechanism](../images/10-identical-particles-fig-05.png)
*Figure 10.5 — Three-step causal chain: symmetric spatial wave function (top track, orange) has high joint probability at $\vec{r}_1=\vec{r}_2$, high Coulomb repulsion, and higher energy; antisymmetric spatial (bottom track, green) has a Pauli node at coincidence, low repulsion, and lower energy.*
...
![Joint probability density: boson, fermion, and distinguishable particles](../images/10-identical-particles-fig-06.png)
*Figure 10.6 — Three panels of $|\psi(x_1,x_2)|^2$ for two particles in a 1D well: bosons (diagonal enhancement), fermions (Pauli node along $x_1=x_2$), and distinguishable particles (no diagonal feature); the marginal density (bottom strip) is identical in all three cases.*
...
**Project:** Build the Atom · **Module this chapter adds:** the antisymmetrization layer — a Slater determinant of spin-orbitals, an exchange-symmetry check, and the Pauli-exclusion test (two identical spin-orbitals → the zero vector). This is the module that enforces "at most one electron per spin-orbital" when the capstone fills shells.

**Tool:** Claude Code (default; Codex CLI or Cowork work identically — determinants and permutations only, pure numpy/itertools).
**Skill level:** Intermediate — the Slater determinant is short; getting the antisymmetry and the exclusion-as-zero right is the point.

**Setup — confirm before you start:**
- [ ] `atom/spin.py` (Chapter 7) exists so spin-orbitals are defined; `atom/hydrogen.py` (Chapter 9) supplies orbital shapes if you want real ones.
- [ ] Python ≥3.10 with numpy.
- [ ] Already covered by the project rule (confirm `CLAUDE.md` carries it).

**The Task — paste into your CLI:**
```
Add the antisymmetrization / Pauli layer. Read atom/spin.py to reuse the spin-orbital labels; do not
modify it. Create ONLY atom/slater.py and tests/test_slater.py. Do not delete anything.

In atom/slater.py implement, with docstrings. Represent a single-particle spin-orbital as a callable
phi(x) where x bundles (position, spin); for the tests you may use simple orthonormal mock orbitals on a
discrete grid so the integrals are exact sums.
  - slater_determinant(orbitals, coords) -> (1/sqrt(N!)) * det[ phi_i(x_j) ]  for N orbitals and N
                                            particle coordinates
  - exchange_sign(psi_func, coords, i, j) -> ratio psi(...x_j...x_i...)/psi(...x_i...x_j...) after swapping
                                             particles i and j (should be -1 for fermions)
  - is_pauli_forbidden(orbitals) -> True if any two orbitals in the list are identical (determinant
                                    vanishes identically)

In tests/test_slater.py write a runnable check (asserts + prints) using two orthonormal mock orbitals
phi_a, phi_b on a small grid:
  (a) ANTISYMMETRY (golden check): for the 2-particle Slater state, print PASS if exchange_sign == -1 to
      1e-12 for a swap of the two particles (P_12 psi = -psi);
  (b) PAULI EXCLUSION AS A THEOREM: build the determinant with two IDENTICAL spin-orbitals (phi_a, phi_a)
      and print PASS if its norm over the grid is < 1e-12 -- identically zero, so not a state;
  (c) NORMALIZATION: print PASS if the (phi_a, phi_b) determinant is normalized to 1 to 1e-6 over the grid;
  (d) EXCHANGE SIGN OF K: verify the exchange integral cross-term is positive for real orbitals and a
      1/|r1-r2|>0 kernel by summing the integrand on the grid; print PASS if K > 0 (orthohelium lies below
      parahelium), and print K.

Then RUN: python tests/test_slater.py  and show me the PASS/FAIL lines. Stop after that.
```

**Expected output:** `atom/slater.py`, `tests/test_slater.py`, and printed lines such as `P₁₂ψ = −ψ (exchange sign −1.000) -> PASS`, `two identical spin-orbitals → ‖det‖=3e-16 (Pauli forbidden) -> PASS`, `‖(φ_a,φ_b) det‖=1.0000 -> PASS`, `exchange integral K=+0.42 > 0 -> PASS`.

**The golden check (what makes this simulation trustworthy):** swapping two particles in the Slater determinant multiplies it by exactly −1 (P₁₂ψ = −ψ to machine precision), and a determinant built from two identical spin-orbitals is the zero vector (norm < 1e-12) — the Pauli exclusion principle as a theorem about determinants, not a postulate. The exchange integral K comes out positive, so the triplet (orthohelium) lies below the singlet.

**What to inspect:** that the antisymmetric sign is exactly −1 (not −1±0.3 from a sloppy grid); that "two electrons in the same 1s spin-orbital" returns the zero vector, which cannot be normalized; that K > 0 sits behind Hund's first rule, which the capstone will invoke.

**If it goes wrong:** the likely failure is a determinant sign convention that reports +1 for a swap (rows vs columns, or an even/odd permutation-parity bug), which breaks the antisymmetry check. Fix the permutation parity; do not force the sign — a wrong exchange sign silently inverts Hund's rule and the ortho/para ordering downstream.
