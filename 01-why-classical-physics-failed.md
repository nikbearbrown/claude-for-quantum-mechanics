...
**Project:** The 1D Quantum Sandbox · **Module this chapter adds:** the energy-scale converters (frequency/wavelength ↔ energy in eV) and a photoelectric-line helper — the machinery the sandbox's eV display is built on.

**Tool:** Claude Code (default; Codex CLI or Cowork work identically — the numerical check runs the same under `node` in any of them).
**Skill level:** Beginner — arithmetic on the energy scale, no differential equations yet.

**Setup — confirm before you start:**

- [ ] `src/constants.js` from Chapter 0 exists and its unit round-trip test passes.
- [ ] Node ≥ 18 (plain ES modules) to run the check.
- [ ] The standing rule is already in `CLAUDE.md` from Chapter 0.

**The Task — paste into your CLI:**

```
Read src/constants.js (do NOT modify it). Create ONLY the new files below; leave every other file untouched.

1. src/energy.js, importing from ./constants.js, exporting:
   - photonEnergyEv(lambda_nm)      // = HC_EV_NM / lambda_nm
   - photonEnergyFromFreqEv(nu_Hz)  // = h*nu converted to eV
   - stoppingPotential(nu_Hz, phi_eV) // Einstein: e*Vstop = h*nu - Phi; return max(0, hnu_eV - phi_eV) in volts, and if h*nu < Phi print a one-line note "sub-threshold: no emission" rather than returning a negative voltage silently.
   This encodes Chapter 1's photoelectric equation K_max = h*nu - Phi.

2. test/check-energy.mjs that:
   - for work functions Phi in {2.28 (Na), 4.30 (Ag), 5.10 (Au)} eV, samples several frequencies above threshold, fits Vstop vs nu by least squares, and asserts the slope equals h/e to < 0.1%. Look up h/e = 4.135667e-15 V*s (CODATA) for the comparison — do NOT hard-code a slope you derived by hand without checking it.
   - asserts the three slopes are equal to each other to < 0.1% (independent of the metal — the Chapter 1 result Millikan confirmed).
   - asserts photonEnergyEv(300) = 4.13 eV to < 0.5%.
   - prints "PASS/FAIL slope = ... V*s (Na/Ag/Au)" and "PASS/FAIL E(300nm) = ... eV".

Then RUN `node test/check-energy.mjs` and show the output. Stop.
```

**Expected output:** `src/energy.js`, `test/check-energy.mjs`, and a printed `PASS slope = 4.1357e-15 V·s, identical for Na/Ag/Au` and `PASS E(300 nm) = 4.13 eV`.

**The golden check (what makes this simulation trustworthy):** the stopping-potential line eVstop = hν − Φ has slope h/e = 4.135667×10⁻¹⁵ V·s (looked-up CODATA), identical for every metal to < 0.1%, and E(λ = 300 nm) = 4.13 eV via hc = 1239.84 eV·nm — a real, parameter-free result from this chapter.

**What to inspect:** that the three fitted slopes coincide (the slope carries no dependence on Φ); that each metal's threshold ν₀ = Φ/h differs; that green light (546 nm) on sodium is flagged sub-threshold rather than returning a negative volt.

**If it goes wrong:** the usual failure is mixing eV and joules inside `stoppingPotential`, which makes the slope come out off by a factor of e (≈ 1.6×10⁻¹⁹). A slope that is a clean constant multiple of the right answer is the tell — trace whether ν was multiplied by h in joules but compared against Φ in eV.
