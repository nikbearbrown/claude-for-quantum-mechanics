...
**Project:** The 1D Quantum Sandbox · **Module this chapter adds:** the integration step — wire every prior module into one `index.html` sandbox with an eigensolver mode and a time-evolution mode, and run the full golden test suite.

**Tool:** Claude Code (default; Codex CLI or Cowork work identically — in Cowork, open the finished sandbox in the built-in preview; the golden suite runs under `node` in all three).
**Skill level:** Advanced — no new physics, but composing eleven modules without regressing any of them is the real test.

**Setup — confirm before you start:**

- [ ] Every prior module exists and passes its own check: `src/constants.js`, `src/energy.js`, `src/grid.js`, `src/wavefn.js`, `src/hamiltonian.js`, `src/timephase.js`, `src/eigensolver.js`, `src/potentials.js`, `src/transmission.js`, `src/harmonic.js`, `src/fft.js`, `src/evolve.js`, `src/observables.js`, `src/measure.js`.
- [ ] Node ≥ 18 (plain ES modules); `mathjs` installed; a browser to view the finished sandbox.
- [ ] The standing rule is in `CLAUDE.md`.

**The Task — paste into your CLI:**

```
Read all src/*.js modules listed above (do NOT rewrite their physics or change their internals). Create ONLY the new files below; leave every existing module untouched.

1. src/sandbox.js that composes the modules into two modes:
   - eigensolver mode: given a potential (from potentials.js or harmonic.js), call solveBound, display energy levels as horizontal lines on the potential plot, eigenfunctions as |psi_n|^2 filled curves offset by E_n, plus the observables panel and a normalization indicator.
   - time-evolution mode: given an initial packet, step with splitStep, animate |psi|^2, keep the normalization indicator live.
   Finish index.html: potential plot, energy levels, offset eigenfunctions, normalization indicator, mode selector.

2. test/golden.test.mjs that runs the full suite and prints one PASS/FAIL line per check plus an overall verdict:
   (1) infinite square well (L=2 nm, m_e, N=500): E2/E1=4.000, E3/E1=9.000, E4/E1=16.000 to < 1%.
   (2) harmonic oscillator: equal spacing dE = hbar*omega to < 1%, E0 = hbar*omega/2.
   (3) orthogonality of eigenstates: |sum_j psi_m* psi_n h| < 1e-8, m != n.
   (4) free Gaussian packet: centroid = x0 + (hbar k0/m) t and width = sigma(t) to < 1%.
   (5) time evolution: sum_j |psi_j|^2 h = 1 to < 1e-6 and <H> drift < 0.1% over the run.
   (6) rectangular barrier: R + T = 1 to < 1e-6.
   End with "GOLDEN SUITE: k/6 PASS".

Then RUN `node test/golden.test.mjs` and show the output, and open index.html to confirm both modes render. Stop.
```

**Expected output:** `src/sandbox.js`, a finished `index.html`, `test/golden.test.mjs`, and a printed six-line report ending `GOLDEN SUITE: 6/6 PASS`, with the sandbox rendering energy levels and a live normalization indicator in the browser.

**The golden check (what makes this simulation trustworthy):** the whole suite passes — infinite-well Eₙ/E₁ = n² to < 1% (E₂/E₁ = 4.000, E₃/E₁ = 9.000), harmonic-oscillator spacing uniform at ℏω to < 1%, eigenstate orthogonality < 1e-8, free-packet centroid and width to < 1%, and under time evolution ∑|ψ|²·h = 1 to < 1e-6 with ⟨Ĥ⟩ conserved to < 0.1%. Every one is an analytic result derived earlier in this book.

**What to inspect:** that the normalization indicator holds 1.000 during evolution; that the ISW ratios read 1, 4, 9, 16; that energy levels render as horizontal lines at the correct Eₙ heights.

**If it goes wrong:** if one module regressed, the suite localizes it — a failed ISW ratio points at the Hamiltonian/eigensolver, a climbing norm points at the split-step stepper, a broken R + T points at the transmission module. Fix the named module and re-run its own chapter check; never loosen a suite tolerance to turn a red line green.

**CLAUDE.md note:** add — "The golden suite (ISW n², HO spacing, orthogonality, norm and ⟨Ĥ⟩ conservation, R+T=1) is the release gate for the sandbox. A green plot with a red suite is a failing build; the physics is confirmed by the suite, not by how the animation looks."
