...
**Project:** Model a Real Quantum System · **Module this chapter adds:** a degenerate-subspace solver (build the perturbation matrix W and diagonalize for good states) plus the hydrogen fine-structure formula, validated against the measured n=2 splitting.

**Tool:** Claude Code (default; Codex CLI or Cowork work identically — identical linear-algebra calls, only the approval UI differs).
**Skill level:** Intermediate — a small eigenvalue problem plus a careful units conversion, and you must locate a real measured splitting to cite.

**Setup — confirm before you start:**

- [ ] The `qm-real-system/` repo, with `methods/perturbation.py` from Chapter 1 in place.
...
```
Add methods/degenerate.py to the qm-real-system repo. Import epsilon.py, validate.py,
report.py. Do not modify Chapter 1's perturbation.py.

Part 1 — the general solver:
  good_states(W): given a Hermitian perturbation matrix W restricted to a degenerate
  subspace, return its eigenvalues (the first-order shifts) and eigenvectors (the good
  zeroth-order states) via numpy.linalg.eigh.
  Test it on the hydrogen n=2 linear Stark block W = e*E*[[0,-3a0],[-3a0,0]]: assert the
  eigenvalues are +/- 3 a0 e E and the trace is 0 (symmetric splitting), matching the
  analytic result in this chapter.

Part 2 — fine structure:
  FIRST estimate the small parameter: fine structure is order alpha^2 relative to the Bohr
  energy, so epsilon = alpha^2 with alpha = 1/137.036. Call epsilon.estimate_epsilon(
  "fine-structure", (1/137.036)**2, "epsilon ~ alpha^2, comparable to Lamb/QED terms") first.
  THEN implement E_fs(n, j) = -(E_n0)^2/(2 m c^2) * (4n/(j+1/2) - 3) with E_n0 = -13.6/n^2 eV
  and m c^2 = 511000 eV. Compute the n=2 levels for j=1/2 and j=3/2 and the interval
  dE = E_fs(2, 1/2) - E_fs(2, 3/2). It should come out near 4.5e-5 eV.

  THEN RUN the validation: I will paste the measured hydrogen 2P_3/2 - 2P_1/2 fine-structure
  interval (look it up and cite author/year; do NOT invent it — it is about 1.09e4 MHz). Convert
  it to eV and call validate.check("H n=2 fine structure", dE_predicted, dE_measured, 0.05, " eV").
  Print a five_move_report whose breakdown line names the Lamb shift (~4e-6 eV, QED) as the
  dominant omitted effect.

STOP and show me methods/degenerate.py, the run output, and the exact datum + citation you
used. Change nothing else.
```

**Expected output:** `methods/degenerate.py`; the Stark block assertions passing (eigenvalues ±3a₀eℰ); `epsilon[fine-structure] = 5.3e-5`; a predicted n=2 interval ≈ 4.5×10⁻⁵ eV; and a `validate.check` PASS line against the cited measured value.

**The golden check (what makes this simulation trustworthy):** the fine-structure formula must give a 2p₃/₂–(2s₁/₂,2p₁/₂) interval of ≈ 4.5×10⁻⁵ eV, which matches the measured hydrogen n=2 fine-structure splitting (the 2P₃/₂–2P₁/₂ interval, ≈ 1.097×10⁴ MHz ≈ 4.5×10⁻⁵ eV — **cite the source you find, do not accept a fabricated number**) to within a few percent. Separately, diagonalizing the Stark W block must return exactly ±3a₀eℰ with zero trace.

**What to inspect:** is ε ≈ α² ≈ 5×10⁻⁵, and is the Lamb shift (≈ 4×10⁻⁶ eV) genuinely about ten times smaller — i.e. is the omitted QED effect small but not negligible? Do the good states from `good_states(W)` come out as the (|2s⟩ ∓ |2p₀⟩)/√2 combinations? Are both fine-structure corrections negative, with 2p₃/₂ sitting *above* the j=½ pair?

**If it goes wrong:** the classic error is the units conversion — dropping the eV↔MHz factor (1 eV ≈ 2.418×10⁸ MHz) makes the percent error absurd; check the datum is in the same units as the prediction before trusting the PASS/FAIL. If the two fine-structure levels come out with the wrong ordering, you have a sign error in the (4n/(j+½)−3) factor — surface it. Do not "adjust" ε: fine structure genuinely sits only one order of magnitude above the Lamb shift, so this ε is small but the QED boundary is close, and the chapter says so.

**CLAUDE.md note:** Already covered by the project rule (the "never invent a measured value — cite it" clause is exactly the fine-structure datum here).
