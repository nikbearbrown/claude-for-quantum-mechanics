...
**Project:** Build the Atom · **Module this chapter adds:** the integration layer — `build_atom(Z)` wires together every prior module into Madelung filling (with the Cr/Cu exceptions), Slater's rules for Z_eff, and Hund's three rules for the term symbol, then validates the whole thing against cited NIST data including the iron golden test ⁵D₄.

**Tool:** Claude Code (default; Codex CLI or Cowork work identically — this is orchestration and table lookup; the only difference is Cowork prints the validation table in its own pane).
**Skill level:** Advanced — this assembles ten chapters of machinery and confronts it with real data, including two elements the model gets wrong.

**Setup — confirm before you start:**
- [ ] All prior modules exist and pass: `atom/angmom.py`, `atom/spin.py`, `atom/coupling.py`, `atom/hydrogen.py`, `atom/slater.py` (Chapters 6–10), plus the Chapter 0 constants.
- [ ] Python ≥3.10 with numpy and scipy.
- [ ] Already covered by the project rule (confirm `CLAUDE.md` carries it) — note especially the clauses on never inventing spectroscopic terms and on flagging where the central-field model breaks (3d/4s, Cr, Cu).

**The Task — paste into your CLI:**
```
Assemble build_atom(Z) and validate it against NIST. Read and reuse the existing modules: atom/spin.py
(spin-orbital doubling), atom/coupling.py (L,S,J coupling), atom/hydrogen.py (Z_eff-scaled orbitals),
atom/slater.py (Pauli exclusion). Do not modify any of them. Create ONLY atom/build_atom.py,
atom/nist_reference.py, and tests/test_capstone.py. Do not delete anything.

In atom/nist_reference.py: create a dict of CITED reference data from the NIST Atomic Spectra Database --
ground-state configuration AND term symbol -- for at least: C(6)=1s2 2s2 2p2, ³P0; O(8)=...2p4, ³P2;
Cr(24)=[Ar]3d5 4s1, ⁷S3; Fe(26)=[Ar]3d6 4s2, ⁵D4; Cu(29)=[Ar]3d10 4s1, ²S1/2. Add a header comment: these
are looked up from NIST, not computed; the human confirms them. Do NOT invent any value -- if unsure of an
entry, mark it "VERIFY AGAINST NIST" rather than guessing.

In atom/build_atom.py implement build_atom(Z) -> (configuration, Z_eff_valence, term_symbol), by:
  1. Madelung (n+ell) filling into subshells of capacity 2(2ell+1), WITH an explicit exceptions table for
     Cr (Z=24 -> [Ar]3d5 4s1) and Cu (Z=29 -> [Ar]3d10 4s1). Comment that these are exchange-stabilization
     exceptions, not derivable from Madelung.
  2. Slater's rules for the valence Z_eff.
  3. Hund's three rules on the open subshell -> S, L, J -> the term symbol ^{2S+1}L_J (S,P,D,F,...).

In tests/test_capstone.py write a runnable validation harness (asserts + prints) that, for each Z in the
reference dict, prints:  Z, element, predicted config, NIST config, MATCH/MISMATCH, predicted term, NIST
term, MATCH/MISMATCH. Then:
  (a) IRON GOLDEN TEST: print PASS only if build_atom(26) returns config [Ar]3d6 4s2 AND term ⁵D4,
      matching NIST exactly;
  (b) ANOMALIES SURFACED: print PASS if Cr and Cu configs match NIST ([Ar]3d5 4s1 and [Ar]3d10 4s1) AND
      the code REPORTS them as Madelung exceptions (not silently) -- if the exceptions table were removed,
      naive Madelung must MISMATCH, and the harness should say so;
  (c) OVERALL: print the count of config matches and term matches out of the reference set, and PASS only
      if carbon ³P0, oxygen ³P2, and iron ⁵D4 all match.

Then RUN: python tests/test_capstone.py  and show me the full validation table and the PASS/FAIL lines.
Stop after that. Do not assert any prediction is correct on your own authority -- the PASS comes only from
matching the cited NIST entries.
```

**Expected output:** `atom/build_atom.py`, `atom/nist_reference.py`, `tests/test_capstone.py`, a printed validation table (one row per element), and lines such as `Fe(26): [Ar]3d⁶4s² ⁵D₄ vs NIST [Ar]3d⁶4s² ⁵D₄ -> MATCH (GOLDEN PASS)`, `Cr,Cu configs match NIST and flagged as exchange exceptions -> PASS`, `configs 5/5, terms 5/5; C ³P₀, O ³P₂, Fe ⁵D₄ -> PASS`.

**The golden check (what makes this simulation trustworthy):** `build_atom(26)` reproduces iron's NIST ground state exactly — configuration [Ar]3d⁶4s², term symbol ⁵D₄ — and the Cr ([Ar]3d⁵4s¹, ⁷S₃) and Cu ([Ar]3d¹⁰4s¹, ²S₁/₂) anomalies match NIST *and* are reported as exchange-stabilization exceptions rather than smoothed over. Carbon ³P₀ and oxygen ³P₂ also match. Every PASS is a match against a cited NIST datum, not the model's self-assessment.

**What to inspect:** the iron row — S=2 (quintet), L=2 (D), J=L+S=4 for the more-than-half-filled 3d⁶ — giving ⁵D₄; that removing the exceptions table makes naive Madelung MISMATCH on Cr and Cu (confirm the harness detects this, proving the exceptions are real, not cosmetic); that the NIST reference file is clearly marked as looked-up, with no invented entries.

**If it goes wrong:** the most likely failure is Hund's Rule 3 giving J=|L−S| instead of L+S for iron — using the less-than-half-filled branch on a more-than-half-filled 3d⁶ subshell — which yields ⁵D₀ instead of ⁵D₄. Fix the filling-fraction test (6 of 10 is more than half); do not hard-code J=4 for iron to pass the test, since that hides the very rule the capstone is validating. And per the project rule: if any predicted term disagrees with NIST, surface the gap as a place the central-field model breaks — do not adjust the prediction to force agreement.

**CLAUDE.md note:** Already covered by the project rule — this capstone is where its clauses on NIST validation, never inventing terms, and flagging the 3d/4s / Cr / Cu breakdowns all earn their place.
