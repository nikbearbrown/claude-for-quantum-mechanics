...
**Project:** Model a Real Quantum System · **Module this chapter adds:** a partial-wave scattering module (phase shifts → cross-section) validated against the exact hard-sphere low-energy limit σ → 4πa².

**Tool:** Claude Code (default; Codex CLI or Cowork work identically — numpy plus scipy special functions).
**Skill level:** Intermediate — a bounded partial-wave sum and one analytic limit to hit.
...
```
Add methods/partial_waves.py to the qm-real-system repo. Import epsilon.py, validate.py,
report.py. Do not modify earlier chapters' modules.

Physics: hard-sphere scattering, radius a, V=inf for r<a. The exact phase shifts are
tan(delta_l) = j_l(ka) / n_l(ka) (spherical Bessel/Neumann); for l=0 this is exactly
delta_0 = -ka. Total cross-section sigma = (4 pi / k^2) sum_l (2l+1) sin^2(delta_l).

1. FIRST estimate the small parameter: the number of contributing partial waves is ~ ka, so
   at low energy epsilon = ka << 1 means only l=0 survives. Call
   epsilon.estimate_epsilon("partial-wave (low-E)", k*a, "epsilon = ka >~ 1 (many waves)").

2. THEN implement sigma_hardsphere(k, a, lmax) summing l = 0..lmax with the exact
   tan(delta_l) above (use scipy.special.spherical_jn / spherical_yn). Assert delta_0 = -ka
   to machine precision.

3. THEN RUN two checks:
   (a) Low-energy limit: at ka = 1e-3, compute sigma and call
       validate.check("hard sphere low-E", sigma, 4*pi*a**2, 0.02, " (a^2)") — expect sigma/(pi a^2) -> 4.
   (b) High-energy limit: at ka = 50 (lmax >= 60), confirm sigma/(pi a^2) -> ~2 (forward
       diffraction) and print it.
   Also verify the optical theorem: sigma == (4 pi / k) * Im f(0) within 1%, where
   f(0) = (1/k) sum_l (2l+1) e^{i delta_l} sin(delta_l). Print a five_move_report; breakdown
   line notes the idealized infinite-wall potential and neglected inelastic channels.

STOP and show me methods/partial_waves.py and the run output. Delete nothing.
```

**Expected output:** `methods/partial_waves.py`; `epsilon[partial-wave (low-E)] = 0.001`; δ₀ = −ka to machine precision; `hard sphere low-E: predicted≈4 measured=4 ... PASS` (σ/πa² → 4); a high-energy σ/πa² ≈ 2; and an optical-theorem consistency line.

**The golden check (what makes this simulation trustworthy):** as ka → 0 the hard-sphere total cross-section must approach **4πa² — four times the geometric πa²** — with δ₀ = −ka exactly, and at ka ≫ 1 it must approach 2πa² (the forward-diffraction result). The optical theorem σ = (4π/k) Im f(0) must hold to ~1% as an independent self-consistency check on the phase-shift sum.

**What to inspect:** is ε = ka ≪ 1 in the low-energy run, and does dropping all ℓ > 0 leave σ unchanged there (only the s-wave matters)? Does the cross-section really sit at 4× geometric, not 1× (the classical value) — the purely quantum factor of four? Does the optical theorem close?

**If it goes wrong:** if σ/πa² comes out ≈ 1 instead of 4, you have used the classical geometric area, or summed |f|² without the (2ℓ+1) weight — surface it. If the high-energy sum doesn't converge to 2, `lmax` is too small: it must exceed ka by a comfortable margin. A failing optical theorem points to a phase error in f(0) (using sin²δ where e^{iδ}sinδ is needed).

**CLAUDE.md note:** Already covered by the project rule (state ε = ka, check against the 4πa² analytic limit and the optical theorem, name the breakdown).
