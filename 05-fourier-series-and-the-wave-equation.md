...
![Momentum-axis diagram of Bloch state components coupled by reciprocal lattice vectors with Brillouin zone boundaries marked](../images/05-fourier-series-and-the-wave-equation-fig-03.png)
*Figure 5.3 — Bloch theorem in momentum space: plane-wave components of a Bloch state at k, k±G, k±2G, connected by coupling brackets indicating coupling by the periodic potential's Fourier components, with zone boundary dashed verticals at ±π/a and a gap indicator between the degenerate pair.*
...
**Project:** The QM Math Toolkit · **Module this chapter adds:** `fourier_series.py` — square-wave Fourier coefficients and partial sums, plus a measurement of the Gibbs overshoot, the mode-decomposition machinery behind the infinite well (I·5) and periodic potentials (III·10).

**Tool:** Claude Code (default; Codex CLI or Cowork work identically — no difference here).
**Skill level:** Intermediate — sine orthogonality and the Gibbs overshoot are the concepts; the numerics are a coefficient loop and a sup-norm.

**Setup — confirm before you start:**

- [ ] `qmtoolkit/probability.py` from Chapter 2 (its `dx`-weighted integration is reused for the orthogonality check).
...
```
Add Fourier-series tools to qmtoolkit. Vol. I Ch. 5 expands an arbitrary square-well state
in sine modes; Vol. III Ch. 10 / band theory expands a periodic potential in harmonics.
Both are projection onto an orthogonal basis.

Create qmtoolkit/fourier_series.py with:
  - sine_overlap(n, m, L=1.0, N=4000): numerically integrate ∫_0^L sin(nπx/L) sin(mπx/L) dx.
  - square_wave_partial_sum(x, N): partial sum through the Nth odd harmonic of the odd
    square wave of period 2π that equals +1 on (0,π) and -1 on (-π,0)
    (coefficients b_k = 4/(kπ) for odd k, zero otherwise).
  - gibbs_overshoot(N): max of the partial sum near the jump minus 1 (the jump half-height),
    i.e. the fractional overshoot for N harmonics.
Leave other files untouched.

Create tests/test_fourier_series.py checking closed-form facts from this chapter:
  1. sine_overlap(2,2) ≈ L/2 and sine_overlap(1,2) ≈ 0 to 1e-6 (orthogonality, (L/2)δ_nm).
  2. As N grows (N=50,200,800), gibbs_overshoot(N) approaches ≈ 0.0895 (about 8.95% of the
     jump) and does NOT shrink toward zero — assert it stays within [0.085, 0.095] for
     N=800. The known Gibbs constant is (1/π)∫_0^π sin t / t dt − 1/2 ≈ 0.08949.

Then RUN `pytest -q`. Print the two overlap values, and the overshoot at each N, with
PASS/FAIL. Do not touch unrelated files. Stop after the pytest summary and numbers.
```

**Expected output:** `qmtoolkit/fourier_series.py` and `tests/test_fourier_series.py` added; `pytest` passes; printed `∫sin2·sin2 = 0.5000 PASS`, `∫sin1·sin2 = 2e-7 PASS`, and an overshoot table `N=50 → 0.0894, N=200 → 0.0895, N=800 → 0.0895 PASS`.

**The golden check (what makes this routine trustworthy):** sine orthogonality `∫_0^L sin(nπx/L)sin(mπx/L)dx = (L/2)δ_{nm}` to `1e-6`, and the Gibbs overshoot at a jump discontinuity converges to ≈**8.95%** of the jump height (the Wilbraham–Gibbs constant `(1/π)∫_0^π (sin t)/t dt − 1/2`) — and crucially does not vanish as more terms are added.

**What to inspect:** that the overshoot is ~0.089 and roughly constant across N (a value that keeps shrinking means you measured pointwise error, not the peak-near-the-jump); that the overshoot peak sits just beside the discontinuity, not at it; that the orthogonality off-diagonal is ~1e-7, not ~0.5.

**If it goes wrong:** if the overshoot comes out ~0.18, the code measured the full jump overshoot against −1 instead of against the half-height +1. If it drifts toward 0, the sampling grid is too coarse to resolve the peak, which narrows (but never shortens) as N grows — refine the grid near `x = 0⁺`.
