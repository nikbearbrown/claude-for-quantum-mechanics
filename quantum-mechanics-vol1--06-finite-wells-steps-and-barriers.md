...
**Project:** The 1D Quantum Sandbox · **Module this chapter adds:** an arbitrary-V(x) builder (step, finite well, rectangular barrier) and the analytic transmission/reflection T(E), R(E) for the rectangular barrier.

**Tool:** Claude Code (default; Codex CLI or Cowork work identically — the sinh/sin branch logic is the same everywhere; check runs under `node`).
**Skill level:** Advanced — the below-barrier (sinh) and above-barrier (sin) branches, and the probability-current bookkeeping that keeps R + T = 1.

**Setup — confirm before you start:**

- [ ] `src/grid.js` (Chapter 2), `src/constants.js` (Chapter 0) exist and pass their checks.
...
1. src/potentials.js, exporting functions that return a Float64Array V over a grid:
   - step(grid, { V0 }); finiteWell(grid, { V0, L }); barrier(grid, { V0, L }).
   V0 in eV, converted to joules internally where needed.

2. src/transmission.js, exporting transmission(E_eV, V0_eV, L_m, mass) -> { T, R, wkbT }:
   - kappa = sqrt(2 m (V0 - E)) / hbar for E < V0.
   - Below barrier: T_exact = [1 + V0^2 sinh^2(kappa L) / (4 E (V0 - E))]^{-1}   (Chapter 6, eq. for T_exact).
   - Above barrier (E > V0): let k2 = sqrt(2 m (E - V0))/hbar and use T_exact = [1 + V0^2 sin^2(k2 L) / (4 E (E - V0))]^{-1}.
   - wkbT = exp(-2 kappa L) (below barrier only).
   - R = 1 - T.

3. test/check-transmission.mjs that:
   - asserts R + T = 1 to < 1e-6 across a sweep of E from 0.1 to 5 * V0.
   - at E = 1 eV, V0 = 5 eV, L = 5 Angstrom, mass = m_e: asserts T_exact ~= 9.1e-5 and T_exact / wkbT = 16 E (V0 - E)/V0^2 = 2.56 to < 1%.
   - above the barrier, asserts T = 1 (within 1e-6) exactly when k2 L = pi (a resonance).
   - prints PASS/FAIL with the numbers.

Then RUN `node test/check-transmission.mjs` and show the output. Stop.
```

**Expected output:** `src/potentials.js`, `src/transmission.js`, `test/check-transmission.mjs`, and printed `PASS R+T = 1`, `PASS T_exact = 9.1e-5`, `PASS T_exact/T_WKB = 2.56`, `PASS above-barrier resonance T = 1 at k₂L = π`.

**The golden check (what makes this simulation trustworthy):** R + T = 1 to < 1e-6 for all E (probability current conserved), the thick-barrier ratio T_exact/T_WKB = 16E(V₀−E)/V₀² (= 2.56 at E = 1 eV, V₀ = 5 eV, giving T_exact ≈ 9.1×10⁻⁵), and T = 1 exactly at the above-barrier resonances k₂L = nπ — all exact results from this chapter.

**What to inspect:** that T rises smoothly through E = V₀; that above-barrier resonances hit T = 1; that doubling L squares e^(−2κL) (T collapses by four orders when κL doubles from ~5 to ~10).

**If it goes wrong:** the signature error is failing to switch branches at E = V₀ — using sinh where sin is required above the barrier makes T exceed 1 or blow up near a resonance. Branch explicitly on E < V₀ vs E > V₀; a T > 1 reading means the wrong hyperbolic/trig function is in play.
