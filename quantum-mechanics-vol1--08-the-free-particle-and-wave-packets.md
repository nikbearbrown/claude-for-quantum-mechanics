...
**Project:** The 1D Quantum Sandbox · **Module this chapter adds:** the split-step Fourier unitary time-evolution stepper (half-V phase → full-T phase in Fourier space → half-V phase), with the correct FFT k-grid.

**Tool:** Claude Code (default; Codex CLI or Cowork work identically — the FFT is written in plain JS, no native library; the check runs under `node`).
**Skill level:** Advanced — a hand-written FFT plus the k-grid sign flip that separates a correct stepper from one that silently corrupts.

**Setup — confirm before you start:**

- [ ] `src/wavefn.js` (Chapter 3), `src/grid.js` (Chapter 2), `src/hamiltonian.js` (Chapter 4, for the ⟨Ĥ⟩ check), `src/constants.js` (Chapter 0) exist and pass their checks.
- [ ] Node ≥ 18 (plain ES modules); a browser to watch the packet evolve.
- [ ] The standing rule is in `CLAUDE.md`.

**The Task — paste into your CLI:**

```
Read src/wavefn.js, src/grid.js, src/hamiltonian.js, src/constants.js (do NOT modify them). Create ONLY the new files below; leave all other files alone. Do NOT use explicit Euler anywhere.

1. src/fft.js: a plain-JS radix-2 in-place FFT and inverse over {re, im} arrays (N a power of 2).

2. src/evolve.js, exporting splitStep(psi, grid, Varray, mass, dt) that advances one step by:
   - multiply psi_j by exp(-i V_j dt / (2 hbar))          // half potential
   - FFT -> psi_hat
   - for each index m, use k_m = (2*pi/(N*h)) * (m < N/2 ? m : m - N)   // REQUIRED sign flip
     multiply psi_hat_m by exp(-i hbar k_m^2 dt / (2 mass))  // full kinetic
   - inverse FFT
   - multiply psi_j by exp(-i V_j dt / (2 hbar))          // half potential
   Each factor has modulus 1, so the step is exactly unitary (Chapter 8 / Chapter 11).

3. test/check-evolve.mjs with V = 0 (free particle), a Gaussian packet (x0, width sigma0, k0), mass = m_e, over many steps:
   - assert the centroid <x>(t) = x0 + (hbar k0 / m) t to < 1%.
   - assert the width sigma_x(t) = sigma0 * sqrt(1 + (hbar t / (2 m sigma0^2))^2) to < 1% (the exact spreading law).
   - assert total probability sum_j |psi_j|^2 h stays 1 to < 1e-6 at every step.
   - assert <H> (via expectationH from hamiltonian.js) drifts by < 0.1% over the run.
   - print PASS/FAIL with the numbers.

Then RUN `node test/check-evolve.mjs` and show the output. Stop.
```

**Expected output:** `src/fft.js`, `src/evolve.js`, `test/check-evolve.mjs`, and printed `PASS centroid = x₀ + (ℏk₀/m)t`, `PASS width matches σ(t)`, `PASS norm = 1.000000 (all frames)`, `PASS ⟨Ĥ⟩ drift < 0.1%`.

**The golden check (what makes this simulation trustworthy):** for a free Gaussian packet the centroid tracks x₀ + (ℏk₀/m)t and the width follows σ(t) = σ₀√(1 + (ℏt/2mσ₀²)²) to < 1%, while ∑|ψ|²·h stays 1 to < 1e-6 and ⟨Ĥ⟩ is conserved to < 0.1% — the exact free-particle dynamics and the unitarity this chapter demands.

**What to inspect:** that the |ψ|² envelope travels at v_g while the internal crests travel at v_g/2; that the momentum distribution |φ(k)|² is unchanged; that the normalization indicator stays flat.

**If it goes wrong:** the classic split-step bug is using the raw FFT index m instead of the sign-flipped k_m, which hands every negative-momentum component the wrong kinetic energy. It looks fine for the first ~10 steps and then grows a grid-scale oscillation in the tails. Apply the k_m sign flip for m ≥ N/2; if the norm climbs instead, you have reintroduced a non-unitary step — never explicit Euler.

**CLAUDE.md note:** add — "Time steppers must be unitary. Explicit Euler is forbidden for the Schrödinger equation; if ∑|ψ|²·h drifts past 1.001 within ten steps the stepper is wrong, not the tolerance — fix the stepper."
