...
**Project:** The QM Math Toolkit · **Module this chapter adds:** `variational.py` — a variational/Rayleigh–Ritz demo that computes `⟨H⟩` for a trial wavefunction in the infinite square well and confirms the upper-bound theorem `E_trial ≥ E_0`, the stationarity machinery of III·3.

**Tool:** Claude Code (default; Codex CLI or Cowork work identically — the kinetic-energy integral can be done symbolically or on a fine grid).
**Skill level:** Advanced — the payoff is that the variational bound is provably an overestimate, checked against an exact closed form.

**Setup — confirm before you start:**

- [ ] `qmtoolkit/probability.py` from Chapter 2 (normalization and `dx`-weighted integrals) and `ode.py` from Chapter 3 (the box energies).
...
```
Add a variational-method demo to qmtoolkit. Vol. III Ch. 3 minimizes <H> over trial
wavefunctions; the minimum is an upper bound on the ground-state energy E_0, and the
stationarity condition is the Schrödinger eigenvalue equation.

Create qmtoolkit/variational.py with:
  - kinetic_expectation(psi_vals, x, m=1.0, hbar=1.0): <T> = (hbar^2/2m) ∫ |psi'|^2 dx for a
    real normalized psi on grid x (integrate |dψ/dx|^2, equivalent to -∫ψ ψ'' after parts).
  - infinite_well_trial_ratio(a=1.0, m=1.0, hbar=1.0): for the parabola trial
    ψ(x)=sqrt(30/a^5) x(a-x) on [0,a] (V=0 inside), compute <H>=<T> and return <H>/E_0
    with E_0 = pi^2 hbar^2/(2 m a^2).
Leave other files untouched.

Create tests/test_variational.py checking closed-form facts from this chapter:
  1. The parabola trial gives <H> = 5 hbar^2/(m a^2) (analytic), so <H>/E_0 = 10/pi^2 ≈ 1.0132;
     assert the computed ratio matches 10/pi^2 to 1e-4 (grid: 4000 points).
  2. The variational bound holds: <H>/E_0 ≥ 1 (it OVERESTIMATES E_0 by ~1.3%).
  DERIVE 10/pi^2 from the closed forms; do not paste 1.0132 as a magic number.

Then RUN `pytest -q`. Print the computed <H>/E_0 beside 10/pi^2, the percentage overestimate,
and the bound check, with PASS/FAIL. Do not touch unrelated files. Stop after the summary.
```

**Expected output:** `qmtoolkit/variational.py` and `tests/test_variational.py` added; `pytest` passes; printed `⟨H⟩/E_0 = 1.01321 vs 10/π² = 1.01321 PASS`, `overestimate = 1.32% PASS`, `bound E_trial ≥ E_0 PASS`.

**The golden check (what makes this routine trustworthy):** for the parabola trial `ψ ∝ x(a−x)` in the infinite well, `⟨H⟩ = 5ℏ²/(ma²)` exactly, so `⟨H⟩/E_0 = 10/π² ≈ 1.0132` to `1e-4`, and the variational theorem guarantees `⟨H⟩ ≥ E_0` (a ~1.3% overestimate, never below).

**What to inspect:** the ratio should be `10/π² ≈ 1.0132` and strictly `≥ 1` (a value below 1 violates the variational theorem and signals a normalization or sign error in `⟨T⟩`); that the trial function was actually normalized before computing `⟨H⟩`; that `⟨T⟩` used `+(ℏ²/2m)∫|ψ'|²` (the integration-by-parts form), matching `−(ℏ²/2m)∫ψψ''`.

**If it goes wrong:** a ratio below 1 usually means `⟨T⟩` was computed from `∫ψψ''` without the minus sign, or `ψ` wasn't normalized (the normalization constant `√(30/a⁵)` is load-bearing). If the ratio is `~4`, the code compared `⟨H⟩` to `E_1` in different unit conventions — keep `ℏ = m = a = 1` consistently.
