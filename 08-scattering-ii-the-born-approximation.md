...
**Project:** Model a Real Quantum System · **Module this chapter adds:** a Born-approximation module (scattering amplitude = Fourier transform of the potential) validated against the exact Rutherford cross-section in the Coulomb limit.

**Tool:** Claude Code (default; Codex CLI or Cowork work identically — numpy plus a 1D quadrature).
**Skill level:** Advanced — you must handle the μ → 0 Coulomb limit and understand why Born is exact there despite a large ε.

**Setup — confirm before you start:**

- [ ] The `qm-real-system/` repo, with `methods/partial_waves.py` from Chapter 7 in place for cross-checks.
...
```
Add methods/born.py to the qm-real-system repo. Import epsilon.py, validate.py, report.py.
Do not modify earlier chapters' modules.

Physics: screened-Coulomb (Yukawa) potential V(r) = (V0/mu) e^{-mu r}/r. The Born amplitude
is f_B(theta) = -(2 m V0)/(hbar^2 mu) * 1/(q^2 + mu^2), q = 2k sin(theta/2). Taking mu->0 with
V0/mu -> Z Z' e^2/(4 pi eps0) gives the Coulomb cross-section, which must equal Rutherford:
  d sigma/d Omega = (Z Z' e^2 / (16 pi eps0 E))^2 / sin^4(theta/2).

1. FIRST estimate the small parameter honestly. The Born parameter is xi = 2 m |V0| / (hbar^2 mu^2)
   (low energy) or 2 m |V0| / (hbar^2 mu k) (high energy). For the alpha-on-gold Coulomb case the
   Sommerfeld parameter eta = Z Z' e^2 / (4 pi eps0 hbar v) ~ 20 >> 1, so Born is NOT justified by a
   small parameter here. Print eta via epsilon.estimate_epsilon("Born (Coulomb eta)", eta,
   "eta >~ 1 — Born unjustified EXCEPT Coulomb, where phases cancel in |f|^2") and print a note
   that Coulomb is the special case where Born reproduces the exact |f|^2 anyway.

2. THEN implement dsigma_born_yukawa(theta, k, m, V0, mu) and dsigma_rutherford(theta, E, ZZp).
   Use alpha (Z=2) on gold (Z'=79) at E = 5.5 MeV; e^2/(4 pi eps0) = 1.44 MeV*fm.

3. THEN RUN the verification:
   (a) Take the Yukawa Born cross-section in the mu -> 0 limit (small mu, e.g. mu = 1e-4 /fm,
       V0/mu set to the Coulomb strength) at theta = 90 deg and compare to Rutherford:
       validate.check("Born->Rutherford (90 deg)", dsig_born, dsig_ruth, 0.02, " fm^2/sr").
   (b) Verify the angular law: fit log(dsigma/dOmega) vs log(sin(theta/2)) over theta in
       [20,160] deg and assert the slope is -4 within 1%.
   Print a five_move_report; breakdown line notes atomic screening (cuts the forward divergence)
   and nuclear-force onset at small impact parameter.

STOP and show me methods/born.py and the run output. Delete nothing.
```

**Expected output:** `methods/born.py`; `epsilon[Born (Coulomb eta)] ≈ 20` with the printed caveat that Coulomb is the exception; a Born-vs-Rutherford PASS at 90° (≈ 4×10² fm²/sr); and a fitted angular slope of −4.

**The golden check (what makes this simulation trustworthy):** in the μ → 0 limit the Born (Yukawa) cross-section must equal the **exact Rutherford formula** $(ZZ'e^2/16\pi\epsilon_0 E)^2/\sin^4(\theta/2)$ to within 2%, and the differential cross-section must follow $\sin^{-4}(\theta/2)$ (log-log slope exactly −4). This agreement is exact — not because ε is small (the Sommerfeld parameter η ≈ 20 is large) but because the Coulomb scattering phases cancel in |f|², a point the chapter makes explicitly.

**What to inspect:** did the code flag that η ≫ 1 so Born is *not* justified by a small parameter, and is Coulomb correctly identified as the special exact case? Is the angular slope −4 (not −2 or −8)? Does the forward cross-section diverge as θ → 0, correctly reflecting the infinite-range potential before screening cuts it off?

**If it goes wrong:** if the slope is −2 you squared sin(θ/2) once too few times (the cross-section goes as 1/q⁴ = 1/sin⁴); surface it. If Born and Rutherford disagree in the μ → 0 limit, your V₀/μ → Coulomb-strength substitution is off — the two constants must be traded together. Do not present the η ≈ 20 case as "Born is valid because ε is small"; it is valid only through the Coulomb phase cancellation, and the code must say so.

**CLAUDE.md note:** Add this method-specific line under the project rule: "The Born approximation reproduces Rutherford exactly for the Coulomb potential despite a large ε (η ≫ 1); this is a phase cancellation in |f|², not a small-parameter justification, and must be labeled as the exception it is."
