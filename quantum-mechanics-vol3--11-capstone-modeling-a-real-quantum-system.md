...
**Project:** Model a Real Quantum System · **Module this chapter adds:** the integration run — the full five-move pipeline on one chosen real system, producing a defended number-with-units and a breakdown report using the harness and method modules from Chapters 0–10.

**Tool:** Claude Code (default; Codex CLI or Cowork work identically — this orchestrates existing modules, no new environment needs).
**Skill level:** Advanced — this is the capstone; it assembles the whole toolkit and demands a defended, cited result.

**Setup — confirm before you start:**

- [ ] The `qm-real-system/` repo with `epsilon.py`, `validate.py`, `report.py`, and the relevant `methods/` module for your chosen system (e.g. `variational.py` for helium, `wkb.py` for STM, `golden_rule.py` for the 2p lifetime, `born.py` for Rutherford).
- [ ] Python ≥3.10 with numpy/scipy.
- [ ] Add the standing rule to `CLAUDE.md` if it isn't there yet.

**The Task — paste into your CLI:**

```
Create capstone.py in the qm-real-system repo. Import epsilon.py, validate.py, report.py, and
the ONE methods/ module for the system I pick. Do not modify any existing module — only
orchestrate them. Do not create files other than capstone.py.

I am modeling this system: [FILL IN one of: (A) STM vacuum-gap tunneling via WKB;
(B) CdSe quantum dot as a 3D spherical box; (C) ammonia inversion 2x2; (D) NMR 1H Rabi pi-pulse;
(E) variational helium ground state; (F) Rutherford alpha-on-gold via Born]. If I leave this
blank, default to (E) variational helium.

Run the five moves IN ORDER and print each as a labelled section via report.five_move_report:
1. System identification: name the degrees of freedom kept and the ones dropped, with a one-line
   justification for dropping each.
2. Method + epsilon: call the chapter method's epsilon estimator FIRST and print epsilon with its
   "breaks-when" condition. If epsilon is not small (or, for the variational case, note it has no
   epsilon, only the bound), say so explicitly — do not proceed as if the method were justified
   when it is not.
3. Calculation: produce the observable as a number WITH UNITS from the model Hamiltonian.
4. Validation: I will paste the measured datum with a citation (author/year). Do NOT invent it.
   Call validate.check with an appropriate tolerance and print percent error + PASS/FAIL.
5. Breakdown: name the two dominant omitted effects and give an order-of-magnitude estimate of
   at least one.

Then write a short defended-number report (<= 300 words) as capstone_report.md summarizing all
five moves, and STOP. Show me capstone.py, capstone_report.md, and the run output.
```

**Expected output:** `capstone.py` plus `capstone_report.md`; a five-move printout ending in a `validate.check` PASS/FAIL line against a cited datum, with an explicit ε (or the variational bound note) and a named, estimated breakdown.

**The golden check (what makes this simulation trustworthy):** the chosen system's prediction must match its cited measured datum with an honest percent error — for the default helium case, E_var = −77.5 eV must be a true **upper bound above −79.0 eV** (cite the source) with ≈1.9% error; for STM, the current drops by ≈7–8 per ångström (2κ ≈ 2.05 Å⁻¹); for the 2p lifetime, ≈1.596 ns; for Rutherford, the exact $\sin^{-4}(\theta/2)$ law. Whichever system, the number is defended only if ε is stated, the datum is cited (never fabricated), and a breakdown magnitude is named.

**What to inspect:** was ε computed and printed *before* the calculation, and does its size actually justify the method (or is it flagged if not — e.g. Born's η ≈ 20 for Coulomb, or the variational bound with no ε)? Is the final number carried with correct units? Is the validation datum real and cited, with the percent error computed rather than asserted?

**If it goes wrong:** the capstone's characteristic failure is a fluent report that skips move 2 or move 5 — a number with no ε and no breakdown reads as authoritative but is exactly the uncontrolled approximation this volume warns against. If the assistant supplies a "measured value" without a citation, stop and look it up yourself; a fabricated datum invalidates the whole defense. For the variational default, any energy below −79.0 eV is a bug (the bound cannot be beaten), not a better result.

**CLAUDE.md note:** Already covered by the project rule — the capstone is where all of it (state ε, cite the datum, name the breakdown, never fabricate, never silently fix) is exercised at once.
