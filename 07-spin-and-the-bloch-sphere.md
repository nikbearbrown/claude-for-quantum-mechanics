...
**Project:** Build the Atom · **Module this chapter adds:** the spin layer — the Pauli algebra, spin along any axis, the Bloch-sphere Born rule P(+) = cos²(γ/2), and the spin-orbital constructor that doubles each spatial orbital into two (mₛ = ±½). This is where an orbital becomes a spin-orbital, the object the Pauli principle counts.

**Tool:** Claude Code (default; Codex CLI or Cowork work identically — pure numpy; a Kronecker product is the only new operation).
**Skill level:** Beginner-to-Intermediate — the Pauli identity is a one-liner to verify; the half-angle in the Born rule is the classic trap.

**Setup — confirm before you start:**
- [ ] `atom/angmom.py` (Chapter 6) exists and its tests pass (spin-½ is the j=½ case of `jmat`).
- [ ] Python ≥3.10 with numpy.
- [ ] Already covered by the project rule (confirm `CLAUDE.md` carries it).

**The Task — paste into your CLI:**
```
Add the spin / spin-orbital layer. Read atom/angmom.py to reuse jmat (spin-1/2 is jmat(0.5)); do not
modify it. Create ONLY atom/spin.py and tests/test_spin.py. Do not delete anything.

In atom/spin.py implement, with docstrings:
  - pauli()                    -> (sx, sy, sz) Pauli matrices
  - spin_along(theta, phi)     -> the operator (hbar/2) n·sigma for unit vector n(theta,phi)
  - spin_up_along(theta, phi)  -> the +hbar/2 eigenstate cos(theta/2)|up> + e^{i phi} sin(theta/2)|down>
  - born_plus(theta_psi, phi_psi, theta_n, phi_n) -> P(+) = cos^2(gamma/2), gamma = angle between the two
                                                     Bloch vectors
  - spin_orbital(spatial_index) -> return the two spin-orbital labels (spatial_index, +1/2) and
                                   (spatial_index, -1/2): a spatial orbital doubles into two spin-orbitals.

In tests/test_spin.py write a runnable check (asserts + prints):
  (a) PAULI ALGEBRA (golden check): print PASS if sigma_i sigma_j == delta_ij I + i eps_ijk sigma_k holds
      for all i,j to 1e-12 (test all 9 products);
  (b) TWO SPOTS EVERY AXIS: for 10 random directions n, print PASS if the eigenvalues of spin_along(n)
      are exactly {-1/2,+1/2}*hbar to 1e-12;
  (c) BORN HALF-ANGLE: print PASS if born_plus returns 1 for aligned analyzers (gamma=0), 0 for
      anti-aligned (gamma=pi), and 1/2 for PERPENDICULAR (gamma=pi/2) -- confirming cos^2(gamma/2), not
      cos^2(gamma); print all three values;
  (d) SPIN DOUBLING: print PASS if spin_orbital(k) yields exactly two labels differing only in m_s, so a
      subshell of ell has capacity 2*(2*ell+1) -- print the capacity for ell=0,1,2 (2,6,10).

Then RUN: python tests/test_spin.py  and show me the PASS/FAIL lines. Stop after that.
```

**Expected output:** `atom/spin.py`, `tests/test_spin.py`, and printed lines such as `σ_iσ_j=δ_ij I + iε_ijk σ_k (9/9) -> PASS`, `n·σ eigenvalues ±½ for 10/10 axes -> PASS`, `P(+): aligned=1.00, anti=0.00, perp=0.50 -> PASS`, `subshell capacities ℓ=0,1,2 → 2,6,10 -> PASS`.

**The golden check (what makes this simulation trustworthy):** the single Pauli multiplication rule σ_iσ_j = δ_ij I + iε_ijk σ_k holds for all nine products to machine precision, n̂·σ has eigenvalues ±½ for every axis (the Stern–Gerlach "two spots, always"), and the Born rule gives exactly ½ at perpendicular analyzers — the signature of the half-angle cos²(γ/2).

**What to inspect:** that perpendicular analyzers give P = ½, not 0 (the whole point of γ/2); that spin doubling yields subshell capacity 2(2ℓ+1) = 2, 6, 10 for s, p, d; that spin-½ built from `jmat(0.5)` equals (ħ/2)σ_i (the algebra of Chapter 6 specializing to spin).

**If it goes wrong:** the signature failure is writing `cos²(γ)` instead of `cos²(γ/2)`, which gives P = 0 at perpendicular — contradicting the 50/50 sequential Stern–Gerlach result. Fix the half-angle; do not rescale γ to force the numbers, since the half-angle also encodes the spinor 4π double cover.
