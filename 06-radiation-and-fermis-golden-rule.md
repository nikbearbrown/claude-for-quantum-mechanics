...
**Project:** Model a Real Quantum System · **Module this chapter adds:** a Fermi-golden-rule / spontaneous-emission calculator — dipole matrix element → Einstein A-coefficient → lifetime — validated against the measured hydrogen 2p → 1s lifetime.

**Tool:** Claude Code (default; Codex CLI or Cowork work identically — scipy for the radial integral, otherwise identical).
**Skill level:** Advanced — a radial integral, an isotropic average, and a careful SI assembly of the A-coefficient.

**Setup — confirm before you start:**

- [ ] The `qm-real-system/` repo, with `methods/tdpt.py` from Chapter 5 in place (golden rule is its continuum limit).
...
```
Add methods/golden_rule.py to the qm-real-system repo. Import epsilon.py, validate.py,
report.py. Do not modify earlier chapters' modules.

Physics: hydrogen 2p -> 1s electric-dipole spontaneous emission.
A21 = omega^3 / (3 pi eps0 hbar c^3) * |<1s| e r |2p>|^2_avg, with the averaged squared
dipole |<1s|e r|2p>|^2_avg = e^2 * 2^15 / 3^10 * a0^2. Photon energy hbar*omega = 10.2 eV.

1. FIRST estimate the small parameter: the electric-dipole approximation holds when
   epsilon = k a0 = (omega/c) a0 << 1. Compute it (expect ~2.6e-3) and call
   epsilon.estimate_epsilon("dipole approx", k*a0, "epsilon = k a0 ~ 1 (hard X-rays)").

2. THEN implement the radial integral R = integral_0^inf R_10(r) r^3 R_21(r) dr with
   R_10 = 2 a0^-3/2 e^{-r/a0}, R_21 = (1/sqrt(24)) a0^-3/2 (r/a0) e^{-r/2a0}, using
   scipy.integrate.quad, and VERIFY it equals the analytic 256/(81 sqrt(6)) a0 to <1%.
   Then assemble the averaged squared dipole and compute A21 (SI) and tau = 1/A21.
   Expect A21 ~ 6.3e8 /s, tau ~ 1.6 ns.

3. THEN RUN the validation. I will paste the measured 2p lifetime (about 1.596 ns — look it
   up and cite author/year; do NOT invent it). Call
   validate.check("H 2p->1s lifetime", tau_predicted, tau_measured, 0.05, " ns").
   Print a five_move_report; breakdown line notes the semiclassical field (the mechanism
   needs QED vacuum fluctuations) and higher multipoles as omitted effects.

STOP and show me methods/golden_rule.py, the run output, and the measured datum + citation.
Delete nothing.
```

**Expected output:** `methods/golden_rule.py`; `epsilon[dipole approx] ≈ 2.6e-3`; a numerically-integrated radial integral matching 256/(81√6) a₀ to <1%; A₂₁ ≈ 6.3×10⁸ s⁻¹; τ ≈ 1.6 ns; and `H 2p->1s lifetime: predicted≈1.6 ns measured≈1.596 ns err<5% PASS`.

**The golden check (what makes this simulation trustworthy):** the computed 2p → 1s lifetime must be ≈ 1.6 ns, matching the measured hydrogen 2p lifetime of ≈ 1.596 ns (cite the source; do not accept a fabricated value) to within a few percent — with the dipole approximation's small parameter ε = ka₀ ≈ 2.6×10⁻³ genuinely tiny. The radial integral must independently reproduce the analytic 256/(81√6) a₀.

**What to inspect:** is ε = ka₀ ≈ 0.003 ≪ 1, confirming the dipole approximation is safe here (and would fail only at ~3.7 keV, ka₀ ≈ 1)? Does the numerical radial integral hit the analytic value, so the SI assembly is the only remaining place for error? Is τ in the right ballpark (nanoseconds, not picoseconds or microseconds — a factor-of-1000 slip signals a unit error)?

**If it goes wrong:** a lifetime off by orders of magnitude is almost always the SI assembly of A₂₁ — check every ε₀, ℏ, c, and the a₀ in meters, and confirm |dipole|² carries e² with the right units. If the radial integral disagrees with 256/(81√6) a₀, the exponents or the r³ measure are wrong; surface it rather than rescaling. The semiclassical derivation cannot explain *why* A₂₁ ≠ 0 — that is QED — so present the number, not a claim that the mechanism is complete.

**CLAUDE.md note:** Already covered by the project rule (cite the measured lifetime; state ε = ka₀; name the QED-mechanism and multipole breakdowns).
