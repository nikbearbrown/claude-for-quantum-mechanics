...
**Project:** Model a Real Quantum System · **Module this chapter adds:** an atoms-in-fields module — Landé g-factors and full intermediate-field Zeeman diagonalization — that reproduces the weak- and strong-field limits and flags the marginal regime.

**Tool:** Claude Code (default; Codex CLI or Cowork work identically — a 6×6 Hermitian eigenproblem in numpy).
**Skill level:** Advanced — building the Hamiltonian in the uncoupled basis with ladder operators and checking two opposite limits.

**Setup — confirm before you start:**

- [ ] The `qm-real-system/` repo, with `methods/degenerate.py` (Chapter 2) available — this reuses the diagonalization discipline.
...
```
Add methods/atoms_fields.py to the qm-real-system repo. Import epsilon.py, validate.py,
report.py. Do not modify earlier chapters' modules.

Physics: hydrogen 2p manifold (l=1, s=1/2), 6 states. Fine structure H_fs = A L.S; Zeeman
H_Z = mu_B B (Lz + 2 Sz)/hbar. Weak field: Delta E = g_J mu_B B m_j with
g_J = 1 + [j(j+1)+s(s+1)-l(l+1)]/[2 j(j+1)]. mu_B = 5.788e-5 eV/T; Delta E_fs(2p) ~ 4.5e-5 eV.

1. FIRST estimate the small parameter for the WEAK-FIELD formula: epsilon = mu_B B / Delta E_fs.
   At B = 0.5 T, mu_B B ~ 2.9e-5 eV, so epsilon ~ 0.65 — NOT small. Call
   epsilon.estimate_epsilon("weak-field Zeeman", mu_B*B/dEfs, "epsilon = mu_B B / dE_fs >~ 0.3")
   and let warn_if_large FIRE. This is a case where the standard formula is suspect and must be
   flagged, not silently used.

2. THEN implement:
   - lande_g(l, s, j) — assert g_J(2p_3/2) = 4/3 and g_J(2p_1/2) = 2/3 exactly.
   - Build the 6x6 H = A*L.S + Zeeman in the |m_l, m_s> uncoupled basis, using
     L.S = Lz Sz + (1/2)(L+ S- + L- S+) with the ladder formulas.
   - eig_zeeman(B): return sorted eigenvalues.

3. THEN RUN the limit checks:
   (a) WEAK field (mu_B B = dEfs/50, epsilon ~ 0.02): eigenvalues must match g_J mu_B B m_j
       relative to the fine-structure centroids — validate.check on the largest 2p_3/2 shift
       (2 mu_B B), tol 0.03.
   (b) STRONG field (mu_B B = 5 dEfs): eigenvalues must approach the Paschen-Back pattern
       mu_B B (m_l + 2 m_s) — validate.check on the extreme level, tol 0.05.
   (c) At B = 0.5 T (epsilon ~ 0.65): print BOTH the weak-field-formula prediction and the exact
       diagonalization, and print their discrepancy as evidence the formula is out of regime.
   Print a five_move_report; breakdown line names the intermediate-regime mixing and the
   neglected nuclear (hyperfine) structure.

STOP and show me methods/atoms_fields.py and the run output. Delete nothing.
```

**Expected output:** `methods/atoms_fields.py`; `g_J(2p₃/₂)=4/3`, `g_J(2p₁/₂)=2/3` exactly; a fired `warn_if_large` at B=0.5 T (ε≈0.65); weak-field PASS and strong-field (Paschen–Back) PASS at the two extremes; and a printed weak-field-vs-exact discrepancy at 0.5 T.

**The golden check (what makes this simulation trustworthy):** the Landé g-factors must be the exact rationals g_J(2p₃/₂) = 4/3 and g_J(2p₁/₂) = 2/3. The 6×6 diagonalization must reproduce the weak-field formula ΔE = g_J μ_B B m_j when ε = μ_B B/ΔE_fs ≪ 1, and the Paschen–Back pattern μ_B B(m_ℓ + 2m_s) when ε ≫ 1 — and at B = 0.5 T, where ε ≈ 0.65, it must show the weak-field formula *failing*, exactly as the chapter warns.

**What to inspect:** does `warn_if_large` actually fire at B = 0.5 T (ε ≈ 0.65), so the marginal regime is flagged rather than trusted? Do the g-factors come out as exact fractions? At B = 0 do the six eigenvalues collapse to the two fine-structure levels (j = ½ and j = 3/2)?

**If it goes wrong:** if the eigenvalues don't approach the weak-field formula even at tiny B, your L·S ladder terms are wrong — verify B = 0 reproduces the fine-structure splitting first. If the code sails past B = 0.5 T reporting a clean weak-field number, the ε guard was skipped: the whole point here is that ε ≈ 0.65 means "suspect," and the assistant must surface that, never present the formula's output as settled.

**CLAUDE.md note:** Already covered by the project rule (the "never apply a method whose ε condition is not satisfied without flagging it" clause is exactly the B = 0.5 T case).
