...
**Project:** The QM Math Toolkit · **Module this chapter adds:** `complex_tools.py` — complex-exponential arithmetic (`cexp`, modulus, conjugate) and phasor addition, so same-frequency oscillations add as one vector in the plane instead of via angle-sum identities.

**Tool:** Claude Code (default; Codex CLI or Cowork work identically — no difference for this pure-numpy module).
**Skill level:** Beginner — the mathematics is Euler's formula and vector addition; the work is packaging it as a tested function.

**Setup — confirm before you start:**

- [ ] The `qmtoolkit` skeleton and passing smoke test from the Introduction chapter.
...
```
Add complex-exponential tools to qmtoolkit. This is the arithmetic Vol. I Ch. 3
(ψ is intrinsically complex) and Ch. 8 (wave packets) use on every page, and Vol. II
Ch. 7 (spinor phase factors) depends on.

Create qmtoolkit/complex_tools.py with:
  - cexp(theta): return np.exp(1j*theta), accepting scalars or arrays (radians).
  - phasor_add(amps): given a list of (magnitude, phase_radians) pairs, return the
    resultant (magnitude, phase) by summing A*exp(i*phi) and taking abs / angle.
Do not modify complex arithmetic conventions: use the physics sign convention and
keep everything in radians. Leave all other files untouched.

Create tests/test_complex_tools.py that checks CLOSED-FORM facts only:
  1. abs(cexp(np.pi) + 1) < 1e-15                       (Euler: e^{iπ}+1 = 0)
  2. np.allclose(np.abs(cexp(theta)), 1.0, atol=1e-15)  for theta = linspace(-10,10,50)
  3. phasor_add([(3,0),(4,np.pi/2)]) returns magnitude ≈ 5 and phase ≈ arctan(4/3)
     to 1e-12 (the 3-4-5 phasor from this chapter's worked example).

Then RUN `pytest -q`. Print each computed residual/value next to its analytic target
and a PASS/FAIL, e.g. "3-4-5 phasor: |A|=5.0000000000 target=5  PASS". Do not delete
unrelated files. Stop after showing the pytest summary and the printed numbers.
```

**Expected output:** `qmtoolkit/complex_tools.py` and `tests/test_complex_tools.py` added; `pytest` reports the new tests passing; printed lines showing the Euler residual, the modulus check, and `|A| = 5.000..., phase = 0.9273 rad (≈53.13°)  PASS`.

**The golden check (what makes this routine trustworthy):** three exact identities from this chapter — `e^{iπ} + 1 = 0` to `1e-15`; `|e^{iθ}| = 1` for all real θ to `1e-15`; and the phasor sum `3∠0° + 4∠90° = 5∠53.13°` (magnitude exactly 5, phase exactly `arctan(4/3)`).

**What to inspect:** that `|e^{iθ}|` stays at 1 across the whole array (not just at θ=0); that the phasor magnitude is 5.000… and not 7 (adding magnitudes instead of vectors) or 1 (subtracting); that the phase came back in radians and matches `arctan(4/3) ≈ 0.9273`.

**If it goes wrong:** if `|A| = 7`, the code added magnitudes as scalars rather than complex numbers — sum `A*np.exp(1j*phi)`, not `A`. If the modulus check fails at large θ, someone worked in degrees; `np.exp(1j*theta)` needs radians.
