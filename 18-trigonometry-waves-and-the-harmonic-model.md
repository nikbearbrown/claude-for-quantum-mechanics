...
**Project:** The QM Math Toolkit · **Module this chapter adds:** `waves.py` — trig/harmonic tools for de Broglie waves: phase and group velocity, two-wave superposition amplitude, and the double-slit interference condition, the oscillation vocabulary of I·2. This is the final routine; running the full suite is the deliverable.

**Tool:** Claude Code (default; Codex CLI or Cowork work identically — no difference here).
**Skill level:** Intermediate — the concepts (v_g = 2·v_ph for matter waves, `d sinθ = nλ`) are exact and checkable; the code is short.

**Setup — confirm before you start:**

- [ ] The full accumulated `qmtoolkit` (Chapters 1–17); this chapter closes the package.
...
```
Add wave/harmonic tools to qmtoolkit and then run the WHOLE test suite. Vol. I Ch. 2
(matter waves) uses the sinusoidal wave ψ = A e^{i(kx-ωt)}: phase vs group velocity,
superposition, and the double-slit condition.

Create qmtoolkit/waves.py with:
  - phase_velocity(k, m=1.0, hbar=1.0): ω/k for the non-relativistic free particle
    ω = ħ k^2/(2m); returns ħk/(2m).
  - group_velocity(k, m=1.0, hbar=1.0): dω/dk = ħk/m (compute analytically or by a central
    finite difference on ω(k) and check they agree).
  - superpose_amplitude(A, delta): resultant amplitude of two equal waves with phase offset
    delta, i.e. 2A|cos(delta/2)|.
  - double_slit_maxima(d, lam, n_max): angles θ with d sinθ = n λ for n=0..n_max.
Leave other files untouched.

Create tests/test_waves.py checking closed-form facts from this chapter:
  1. For a non-relativistic matter wave, group_velocity / phase_velocity = 2 exactly
     (to 1e-12), for several k — the envelope moves at twice the phase pattern's speed.
  2. superpose_amplitude(A, 0) = 2A (constructive), superpose_amplitude(A, π) = 0
     (destructive), and superpose_amplitude(1, π/3) = sqrt(3) to 1e-12 (|ψ1+ψ2|^2 = 3).
  3. double_slit_maxima: the n=1 maximum satisfies sin(θ) = λ/d to 1e-12.
  4. Identity: cos^2 + sin^2 = 1 to 1e-14 across a grid (sanity of the trig core).

Then RUN THE ENTIRE SUITE: `pytest -q` across all of tests/. Print v_g/v_ph, the three
superposition amplitudes, the n=1 slit angle, and — as the capstone — the total number of
tests collected and passed, with PASS/FAIL. Do not touch unrelated files. Stop after the
full-suite summary.
```

**Expected output:** `qmtoolkit/waves.py` and `tests/test_waves.py` added; the **entire** `qmtoolkit` suite runs green; printed `v_g/v_ph = 2.0000000000 PASS`, `amplitudes: 2A, 0, √3 PASS`, `sinθ_1 = λ/d PASS`, and a capstone line `NN passed — qmtoolkit suite green`.

**The golden check (what makes this routine trustworthy):** exact wave facts from this chapter — for a non-relativistic de Broglie wave `v_g/v_ph = 2` (since `ω = ℏk²/2m`); the two-wave superposition amplitude is `2A|cos(δ/2)|`, giving `2A` at `δ=0`, `0` at `δ=π`, and `√3` at `δ=π/3`; and the double-slit maxima obey `d sinθ = nλ` — plus `sin²+cos²=1` to `1e-14`.

**What to inspect:** `v_g/v_ph` must be exactly 2, not 1 (non-dispersive assumption) or 0.5 (ratio inverted); the analytic and finite-difference group velocities should agree; the superposition amplitude at `δ=π` must be 0, not `2A` (a dropped factor of `½` inside the cosine); the n=1 slit angle satisfies `sinθ = λ/d`, requiring `λ < d`.

**If it goes wrong:** if `v_g/v_ph = 1`, someone used a linear dispersion `ω = vk` instead of the matter-wave `ω = ℏk²/2m`. If the finite-difference `dω/dk` disagrees with the analytic form, the step is too large or `ω(k)` was evaluated in the wrong units — keep `ℏ = m = 1`. A `double_slit_maxima` that returns NaN means `nλ/d > 1` (no real angle); guard the domain.
