...
**Project:** The QM Math Toolkit · **Module this chapter adds:** `special.py` — Hermite and Legendre polynomial evaluators with their weighted orthogonality integrals, verifying the Sturm–Liouville orthogonality that names the oscillator (I·7), angular-momentum (II·6), and hydrogen (II·9) eigenstates.

**Tool:** Claude Code (default; Codex CLI or Cowork work identically — you may use `numpy.polynomial.hermite`/`legendre` or `scipy.special`, but the orthogonality integral is the deliverable).
**Skill level:** Intermediate–Advanced — the key idea is the weight function: orthogonality only holds against `e^{−ξ²}` (Hermite) or `1` on `[−1,1]` (Legendre).

**Setup — confirm before you start:**

- [ ] `qmtoolkit/probability.py` from Chapter 2 (its weighted integration is reused with a nontrivial weight).
...
```
Add special-function orthogonality tools to qmtoolkit. Vol. I Ch. 7 (Hermite/oscillator),
Vol. II Ch. 6 (Legendre/angular momentum), and Ch. 9 (hydrogen) all rely on one fact:
each family is orthogonal under its own Sturm–Liouville weight.

Create qmtoolkit/special.py with:
  - hermite_H(n, xi): the PHYSICIST Hermite polynomial (leading coeff 2^n).
  - legendre_P(n, u): the Legendre polynomial on [-1,1].
  - hermite_overlap(m, n): ∫_{-∞}^{∞} H_m H_n e^{-ξ^2} dξ, by high-accuracy quadrature
    (scipy.integrate.quad or Gauss–Hermite).
  - legendre_overlap(m, n): ∫_{-1}^{1} P_m P_n du.
Leave other files untouched.

Create tests/test_special.py checking the closed-form orthogonality relations from this chapter:
  1. hermite_overlap(m,n) = sqrt(pi) * 2^n * n! * δ_mn to 1e-8 for m,n in 0..4 (weight e^{-ξ^2}).
  2. legendre_overlap(m,n) = 2/(2n+1) * δ_mn to 1e-10 for m,n in 0..5.
  3. Sanity of the polynomials themselves: H_2(ξ) = 4ξ^2 - 2 and P_2(u) = (3u^2-1)/2 to 1e-12
     at sample points. DERIVE the diagonal targets from the boxed formulas; do not paste decimals.

Then RUN `pytest -q`. Print, for a few (m,n) pairs, the computed overlap beside
sqrt(pi)2^n n! δ_mn (Hermite) and 2/(2n+1) δ_mn (Legendre), with PASS/FAIL. Do not touch
unrelated files. Stop after the pytest summary and numbers.
```

**Expected output:** `qmtoolkit/special.py` and `tests/test_special.py` added; `pytest` passes; printed rows like `⟨H_2|H_2⟩ = 8√π = 14.1796 PASS`, `⟨H_1|H_2⟩ = 3e-12 PASS`, `⟨P_3|P_3⟩ = 2/7 = 0.285714 PASS`.

**The golden check (what makes this routine trustworthy):** the exact Sturm–Liouville orthogonality integrals of this chapter — `∫H_m H_n e^{−ξ²}dξ = √π·2ⁿ·n!·δ_{mn}` (weight `e^{−ξ²}`) to `1e-8`, and `∫_{−1}^{1}P_m P_n du = 2/(2n+1)·δ_{mn}` (weight 1) to `1e-10`.

**What to inspect:** the off-diagonal overlaps should be ~1e-10 (a nonzero off-diagonal usually means the weight `e^{−ξ²}` was omitted, so the integral diverges or misbehaves); the Hermite diagonal must carry the `2ⁿ n!` factor (using the probabilist's `He_n` with weight `e^{−ξ²/2}` shifts every value by powers of 2); the Legendre diagonal is `2/(2n+1)`, not 1.

**If it goes wrong:** if the Hermite integral blows up or fails to converge, the code integrated `H_m H_n` without the Gaussian weight — Gauss–Hermite quadrature already includes `e^{−ξ²}`, so don't multiply it in twice (a doubled weight is as wrong as a missing one). If diagonals are off by powers of 2, you mixed physicist and probabilist Hermite conventions.
