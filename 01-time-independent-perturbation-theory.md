...
**Project:** Model a Real Quantum System · **Module this chapter adds:** a non-degenerate perturbation-theory solver — first- and second-order energy corrections from matrix elements — validated against exact diagonalization.

**Tool:** Claude Code (default; Codex CLI or Cowork work identically — the numpy/scipy calls are identical, only the CLI's edit-confirmation flow differs).
**Skill level:** Beginner–Intermediate — one matrix element and a bounded sum, but you must build the exact-diagonalization check yourself.

**Setup — confirm before you start:**

- [ ] The `qm-real-system/` repo from Chapter 0, with `epsilon.py`, `validate.py`, and `report.py` working (the smoke test passes).
...
```
Add methods/perturbation.py to the qm-real-system repo. Do not modify epsilon.py,
validate.py, or report.py — import them. Do not touch other chapters' modules.

Physics: the quartic oscillator H = p^2/2m + (1/2) m w^2 x^2 + lambda*x^4, in natural
units hbar = m = w = 1, with H' = x^4. Build x = sqrt(1/2)(a_+ + a_-) as a matrix in the
harmonic-oscillator number basis truncated at N=200 levels; form x^4 by matrix powers.

1. FIRST estimate the small parameter. For the ground state the first-order shift is
   E0^(1) = (3/4) lambda, and the controlling ratio is epsilon = |E0^(1)| / (hbar*w) = (3/4)*lambda
   (correction relative to the level spacing). Call epsilon.estimate_epsilon("nondegen-PT",
   0.75*lambda, "epsilon >~ 0.3") and epsilon.warn_if_large(...) BEFORE computing anything else.

2. THEN implement the method as reusable functions:
   - first_order(H0_diag, Hp, n): returns <n|Hp|n>.
   - second_order(H0_diag, Hp, n): returns sum_{m!=n} |<m|Hp|n>|^2 / (E_n^0 - E_m^0).
   Compute E0 ~= E0^(0) + lambda*first_order + lambda^2*second_order for the ground state
   at lambda = 0.02.

3. THEN RUN the verification: numerically diagonalize the full truncated H(lambda=0.02),
   take its lowest eigenvalue E0_exact, and call
   validate.check("quartic ground state", E0_PT, E0_exact, 0.05, " (hbar*w)").
   Also assert second_order(...) for the ground state is < 0 (the theorem in this chapter).
   Print a five_move_report. Then sweep lambda in {0.02, 0.1, 0.3, 0.6} and print epsilon and
   the PT-vs-exact percent error for each so the breakdown as epsilon -> 1 is visible.

4. STOP and show me methods/perturbation.py and the run output. Delete nothing.
```

**Expected output:** `methods/perturbation.py`; a printed `epsilon[nondegen-PT] = 0.015` at λ=0.02, a five-move report, and `quartic ground state: predicted≈0.5075 ... measured≈0.5075 ... err<5% PASS`, plus a λ-sweep table showing the error climbing as ε grows.

**The golden check (what makes this simulation trustworthy):** at λ=0.02 the first-plus-second-order PT ground-state energy must match the exact numerical eigenvalue of the truncated quartic Hamiltonian to well under 5%, while ε = ¾λ = 0.015 ≪ 1. Independently, the second-order ground-state correction must be **negative** — this chapter proves it is negative for *any* perturbation, so a positive value is a guaranteed bug.

**What to inspect:** is ε actually small (0.015) at λ=0.02, and does the PT-vs-exact error track ε as you raise λ? Does the second-order correction come out negative? At λ=0.6 (ε=0.45) does the error balloon, confirming the method is out of its regime?

**If it goes wrong:** if PT and exact disagree even at tiny λ, your basis is too small — the sum needs enough levels that ⟨m|x⁴|n⟩ has decayed; raise N and re-check convergence. If the second-order correction is positive, you have the energy denominator sign flipped ($E_n^{(0)}-E_m^{(0)}$, not the reverse) — surface it, do not silently negate. Applying first-order PT at λ≳0.4 where ε≈0.3 will give a large, wrong-signed-looking discrepancy; that is the method failing, not the code.

**CLAUDE.md note:** Already covered by the project rule (state ε, check against the analytic/exact limit, name the breakdown).
