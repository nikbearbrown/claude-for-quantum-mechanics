...
**Project:** The 1D Quantum Sandbox · **Module this chapter adds:** the eigensolver — diagonalize the tridiagonal Hamiltonian to return the bound-state energies Eₙ and h-normalized eigenfunctions ψₙ. This is the heart of the whole sandbox.

**Tool:** Claude Code (default; Codex CLI or Cowork work identically — all install `mathjs` and run the same `node` check).
**Skill level:** Intermediate — the physics is one library call; the discipline is enforcing Dirichlet walls and h-normalizing the result.

**Setup — confirm before you start:**

- [ ] `src/hamiltonian.js` (Chapter 4), `src/grid.js` (Chapter 2), `src/wavefn.js` (Chapter 3) exist and pass their checks.
- [ ] Node ≥ 18 (plain ES modules); `npm install mathjs` (sanctioned by the project for `math.eigs`).
- [ ] The standing rule is in `CLAUDE.md`.

**The Task — paste into your CLI:**

```
Read src/hamiltonian.js, src/grid.js, src/wavefn.js, src/constants.js (do NOT modify them). Create ONLY the new files below; leave all other files alone.

1. src/eigensolver.js, exporting solveBound(grid, Varray, mass, nEig):
   - Build the (N-2) x (N-2) INTERIOR tridiagonal matrix only, excluding the two boundary points (this enforces the Dirichlet walls psi_0 = psi_{N-1} = 0 from Chapter 5).
   - Call math.eigs on it, sort eigenpairs by ascending energy, take the lowest nEig.
   - h-normalize each eigenvector: divide by sqrt(sum_j |psi_j|^2 * h) so sum_j |psi_j|^2 * h = 1. (math.eigs returns Euclidean-normalized vectors; the *h matters.)
   - Return { energies_eV: [...], states: [...] }.

2. test/check-eigensolver.mjs for the infinite square well (V = 0 inside, hard walls), L = 2 nm, mass = m_e, N = 500:
   - assert E2/E1 = 4, E3/E1 = 9, E4/E1 = 16 each to < 0.5% (the Chapter 5 result E_n/E1 = n^2).
   - assert E1 = pi^2 hbar^2 / (2 m_e L^2) ~= 0.094 eV to < 1%.
   - assert each state is h-normalized to 1 within 1e-6 and orthogonality |sum_j psi_m* psi_n h| < 1e-8 for m != n.
   - assert state n has n-1 interior nodes.
   - print a table (n, E_n, E_n/E1, nodes) and PASS/FAIL lines.

Then RUN `node test/check-eigensolver.mjs` and show the output. Stop.
```

**Expected output:** `src/eigensolver.js`, `test/check-eigensolver.mjs`, and a printed table with `E₂/E₁ = 4.000`, `E₃/E₁ = 9.000`, `E₄/E₁ = 16.000`, `E₁ ≈ 0.094 eV`, node counts 0,1,2,3,4, and `PASS` on ratios, absolute energy, normalization, and orthogonality.

**The golden check (what makes this simulation trustworthy):** the infinite-square-well ratios Eₙ/E₁ = n² to < 1% (E₂/E₁ = 4.000, E₃/E₁ = 9.000, E₄/E₁ = 16.000 to three decimals at N = 500), with no fitting parameters — the book's non-negotiable acceptance test.

**What to inspect:** the ratios read 1, 4, 9, 16, 25; ψₙ has exactly n − 1 interior nodes; every returned state h-normalizes to 1.

**If it goes wrong:** two distinct failure modes, distinguishable by the numbers. If the ratios themselves are off (E₂/E₁ ≠ 4), the boundary points were not excluded, so the Dirichlet walls were never enforced — build only the interior (N−2)×(N−2) block. If the ratios are right but the norm reads ~250, the eigenvectors are Euclidean-normalized, not h-normalized — divide each by √h. Surface which one; do not tune tolerances to hide it.

**CLAUDE.md note:** add — "The infinite-square-well ratio test Eₙ/E₁ = n² (to < 1%) is the sandbox's non-negotiable acceptance gate: no change to the Hamiltonian or eigensolver ships until this test is green."
