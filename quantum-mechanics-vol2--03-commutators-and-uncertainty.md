...
**Project:** Build the Atom · **Module this chapter adds:** the commutator/compatibility layer — a `commutator` routine, a compatibility test, and a Robertson-bound checker. This is the machinery that will certify a Complete Set of Commuting Observables (CSCO), the algebraic license for labeling every orbital by (n, ℓ, mℓ, mₛ).

**Tool:** Claude Code (default; Codex CLI or Cowork work identically — the ℓ=1 matrices are small and deterministic in all three).
**Skill level:** Intermediate — matrix commutators are easy to code but the sign of σ_y and the operator ordering are classic traps.

**Setup — confirm before you start:**
- [ ] `atom/states.py` (Chapter 1) and `atom/observables.py` (Chapter 2) exist and their tests pass.
- [ ] Python ≥3.10 with numpy.
- [ ] Already covered by the project rule (confirm `CLAUDE.md` carries it).

**The Task — paste into your CLI:**
```
Add the commutator/compatibility layer. Read atom/states.py and atom/observables.py to reuse adjoint,
expval, and diagonalize; do not modify them. Create ONLY atom/commutators.py and tests/test_commutators.py.
Do not delete anything.

In atom/commutators.py implement, with docstrings:
  - commutator(A, B)           -> A@B - B@A
  - compatible(A, B, tol=1e-12)-> True if [A,B] == 0 (share an eigenbasis)
  - robertson_lhs(A, B, psi)   -> sigma_A * sigma_B  (product of standard deviations)
  - robertson_rhs(A, B, psi)   -> 0.5 * |<[A,B]>|
Provide the explicit ell=1 angular-momentum matrices Lx, Ly, Lz and L2 = Lx@Lx+Ly@Ly+Lz@Lz (in units of
hbar=1), in the basis |1,-1>,|1,0>,|1,1>, exactly as given in Chapter 6's matrices.

In tests/test_commutators.py write a runnable check (asserts + prints) grounded in this chapter's algebra:
  (a) SPIN ALGEBRA CLOSES: with Sx,Sy,Sz = Pauli/2, print PASS if commutator(Sx,Sy) == 1j*Sz to 1e-12
      (and cyclically) -- this is [S_i,S_j]=i eps_ijk S_k;
  (b) CSCO CHECK: print PASS if compatible(L2, Lz) is True (||[L²,Lz]|| < 1e-12) but compatible(Lx, Ly)
      is False -- L² and Lz can jointly label states; Lx and Ly cannot;
  (c) ROBERTSON HOLDS: for 20 random normalized qubit states psi, print PASS if
      robertson_lhs(Sx,Sy,psi) >= robertson_rhs(Sx,Sy,psi) - 1e-12 for every one; print the tightest case.

Then RUN: python tests/test_commutators.py  and show me the PASS/FAIL lines. Stop after that.
```

**Expected output:** `atom/commutators.py`, `tests/test_commutators.py`, and printed lines such as `[Sx,Sy]=iSz, [Sy,Sz]=iSx, [Sz,Sx]=iSy -> PASS`, `[L²,Lz]=0 (compatible), [Lx,Ly]≠0 (incompatible) -> PASS`, `Robertson σ_Aσ_B ≥ ½|⟨[A,B]⟩| for 20/20 states -> PASS`.

**The golden check (what makes this simulation trustworthy):** the spin algebra closes exactly — [Sₓ,S_y] = iħS_z (and its two cyclic partners) to machine precision — and the CSCO fact [L²,L_z] = 0 while [Lₓ,L_y] ≠ 0 holds to 1e-12. These are the operator identities that make (n, ℓ, mℓ, mₛ) simultaneously well-defined.

**What to inspect:** the antisymmetry [A,B] = −[B,A] (print both and confirm); that σ_y = [[0,−i],[i,0]] and is Hermitian (a flipped sign here quietly reverses the commutator); that `compatible` returns a crisp True/False, not a near-zero float you have to eyeball.

**If it goes wrong:** the most likely failure is a sign error in σ_y giving [Sₓ,S_y] = −iS_z, which fails check (a). Do not silently flip the sign to make the test pass — surface it and re-derive σ_y from σ_x, σ_z via the Pauli anticommutation, since a wrong σ_y will corrupt every angular-momentum result downstream.
