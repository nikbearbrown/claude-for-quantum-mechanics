...
**Project:** The QM Math Toolkit · **Module this chapter adds:** `inner_product.py` — the physicist's complex inner product (conjugate-in-the-first-slot) and Gram–Schmidt orthonormalization, the Hilbert-space scaffolding of II·1 and IV·2.

**Tool:** Claude Code (default; Codex CLI or Cowork work identically — no difference here).
**Skill level:** Intermediate — the one trap is the conjugate on the bra; get it wrong and complex vectors fail to orthonormalize.

**Setup — confirm before you start:**

- [ ] `qmtoolkit/complex_tools.py` from Chapter 1 (complex arithmetic underlies the inner product).
...
```
Add inner-product and Gram–Schmidt tools to qmtoolkit. Vol. II Ch. 1 (Hilbert-space
formalism) and Vol. IV Ch. 2 (entanglement / Schmidt decomposition) are built on the
complex inner product and orthonormal bases.

Create qmtoolkit/inner_product.py with:
  - inner(phi, psi): the PHYSICIST convention <phi|psi> = sum(conj(phi)*psi) — conjugate
    on the FIRST argument. Do not drop the conjugate.
  - gram_schmidt(vectors): given a list/array of independent complex column vectors, return
    an orthonormal set using inner() for projections.
Leave other files untouched.

Create tests/test_inner_product.py checking closed-form facts from this chapter:
  1. Conjugate symmetry: for random complex vectors, inner(phi,psi) == conj(inner(psi,phi))
     to 1e-14.
  2. Gram–Schmidt on the chapter's C^2 example u1=(1,i), u2=(1,0) returns e1=(1,i)/√2 and
     e2=(1,-i)/√2; assert the output Gram matrix G_ij = inner(e_i, e_j) equals the identity
     to 1e-12 (orthonormal to machine precision).
  3. Run Gram–Schmidt on a random 5×5 complex matrix's columns; assert the Gram matrix
     equals I to 1e-12.

Then RUN `pytest -q`. Print the max off-diagonal of each Gram matrix and the conjugate-
symmetry residual, with PASS/FAIL. Do not touch unrelated files. Stop after the summary.
```

**Expected output:** `qmtoolkit/inner_product.py` and `tests/test_inner_product.py` added; `pytest` passes; printed `Gram off-diagonal max = 4e-16 PASS` for both the C² example and the 5×5 case, and `conjugate-symmetry residual = 0 PASS`.

**The golden check (what makes this routine trustworthy):** Gram–Schmidt output is orthonormal to machine precision — the Gram matrix `G_{ij} = ⟨e_i|e_j⟩` equals the identity `I` to `1e-12` — and the inner product is conjugate-symmetric `⟨φ|ψ⟩ = ⟨ψ|φ⟩*` to `1e-14`.

**What to inspect:** the Gram-matrix off-diagonals should be ~1e-16, not ~0.5 (a nonzero off-diagonal means the conjugate was dropped, exactly the `½(1+1) ≠ 0` failure in this chapter's worked example); the diagonals should be exactly 1 (normalization); `⟨ψ|ψ⟩` should be real and positive for every complex input.

**If it goes wrong:** if the C² check fails with off-diagonal ≈0.5, the code used `sum(phi*psi)` without conjugation — the norm `⟨ψ|ψ⟩` would then be complex, a guaranteed tell. Surface it rather than patching the tolerance.
