...
**Project:** Build the Atom · **Module this chapter adds:** the angular-momentum coupling layer — build the coupled |J,M⟩ basis from two angular momenta via the total-J operators, get the Clebsch–Gordan matrix, and read off L, S, J. This is the machinery that couples an open subshell's electrons into the total L and S a term symbol reports.

**Tool:** Claude Code (default; Codex CLI or Cowork work identically — you may use `sympy.physics.quantum.cg` for a cross-check; it ships with all three).
**Skill level:** Advanced — assembling J² = J₁² + J₂² + 2J₁·J₂ from tensor products and diagonalizing it is the first genuinely composite construction.

**Setup — confirm before you start:**
- [ ] `atom/angmom.py` (Chapter 6) and `atom/observables.py` (Chapter 2) exist and their tests pass.
- [ ] Python ≥3.10 with numpy (sympy optional, for the CG cross-check).
- [ ] Already covered by the project rule (confirm `CLAUDE.md` carries it).

**The Task — paste into your CLI:**
```
Add the angular-momentum coupling layer. Read atom/angmom.py (reuse jmat) and atom/observables.py (reuse
diagonalize); do not modify them. Create ONLY atom/coupling.py and tests/test_coupling.py. Do not delete
anything.

In atom/coupling.py implement, with docstrings (units hbar=1), using tensor products (numpy.kron):
  - total_operators(j1, j2)    -> Jx,Jy,Jz,J2 for the combined system, where e.g.
                                  Jz = kron(Jz1, I2) + kron(I1, Jz2) and J2 = Jx^2+Jy^2+Jz^2
  - allowed_J(j1, j2)          -> the triangle-rule list |j1-j2| ... j1+j2
  - coupled_basis(j1, j2)      -> simultaneous eigenstates of J2 and Jz (the |J,M> vectors), obtained by
                                  diagonalizing J2 within each M-sector; return them labeled by (J,M).
  - cg_matrix(j1, j2)          -> the unitary matrix of Clebsch-Gordan coefficients relating uncoupled to
                                  coupled basis.

In tests/test_coupling.py write a runnable check (asserts + prints):
  (a) STATE COUNT: for (1/2,1/2), (1,1/2), (1,1) print PASS if sum(2J+1 over allowed_J) ==
      (2j1+1)(2j2+1) -- 4, 6, 9 respectively;
  (b) SINGLET HAS J=0 (golden check): for two spin-1/2, print PASS if the state
      (|up,down> - |down,up>)/sqrt(2) is an eigenstate of J2 with eigenvalue 0 to 1e-12, and the triplet
      |1,0> = (|up,down>+|down,up>)/sqrt(2) has J2 eigenvalue 2 (=1*(1+1)); print both eigenvalues;
  (c) CG UNITARITY / COMPLETENESS: print PASS if cg_matrix is unitary to 1e-12 (each coupled state's
      squared coefficients sum to 1) for all three (j1,j2) pairs.

Then RUN: python tests/test_coupling.py  and show me the PASS/FAIL lines. Stop after that.
```

**Expected output:** `atom/coupling.py`, `tests/test_coupling.py`, and printed lines such as `state counts: 4=4, 6=6, 9=9 -> PASS`, `J²(singlet)=0.000, J²(triplet|1,0⟩)=2.000 -> PASS`, `CG unitary (Σ|c|²=1) for all pairs -> PASS`.

**The golden check (what makes this simulation trustworthy):** the spin singlet (|↑↓⟩ − |↓↑⟩)/√2 is an eigenstate of J² with eigenvalue exactly 0, while the triplet |1,0⟩ = (|↑↓⟩ + |↓↑⟩)/√2 has J² eigenvalue 2 = 1(1+1) — the one minus sign that separates J=0 from J=1 — and the Clebsch–Gordan matrix is unitary (completeness: squared coefficients sum to 1) to machine precision.

**What to inspect:** the state-count identity Σ(2J+1) = (2j₁+1)(2j₂+1) before any coupling algebra; that J² = J₁²+J₂²+2J₁·J₂ includes the cross term (omitting it gives the wrong eigenvalue on the singlet); the exchange symmetry — singlet antisymmetric, triplet symmetric — which feeds directly into the Pauli bookkeeping of Chapter 10.

**If it goes wrong:** the likeliest failure is dropping the 2J₁·J₂ cross term (or the J₁₊J₂₋ + J₁₋J₂₊ flip-flop part of it), which leaves J² diagonal in the uncoupled basis and reports the singlet as J²≠0. Fix by building J² from the full total operators via `kron`, not as J₁²+J₂²; surface any factor-of-2 discrepancy rather than tuning it away.
